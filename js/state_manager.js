...
const StateManager = (() => {
    ...
    // Imported-channel enrichment is owned by the background worker. The
    // dashboard only sends commands and observes the persisted job status, so
    // closing this page does not destroy a pending CSV/TXT/JSON queue.
    ...
    const sendImportedChannelEnrichmentCommand = (action, payload = {}) => new Promise((resolve) => {
        const runtimeApi = (typeof browser !== 'undefined' && browser.runtime && typeof browser.runtime.sendMessage === 'function')
            ? browser
            : ((typeof chrome !== 'undefined' && chrome.runtime && typeof chrome.runtime.sendMessage === 'function') ? chrome : null);
        if (!runtimeApi) {
            resolve({ ok: false, error: 'background_unavailable', errorCode: 'background_unavailable' });
            return;
        }
        ...
        const finish = (response) => {
            if (settled) return;
            settled = true;
            resolve(response || { ok: false, error: 'background_unavailable', errorCode: 'background_unavailable' });
        };
        try {
            const message = { action, ...payload };
            const maybePromise = typeof browser !== 'undefined' && runtimeApi === browser
                ? runtimeApi.runtime.sendMessage(message)
                : runtimeApi.runtime.sendMessage(message, finish);
            ...
            finish({
                ok: false,
                error: error?.message || 'background_unavailable',
                errorCode: 'background_unavailable'
            });
        }
    });

    const startImportedChannelEnrichment = (options = {}) => sendImportedChannelEnrichmentCommand(
        'FilterTube_StartImportedChannelEnrichment',
        Array.isArray(options?.profileIds) ? { profileIds: options.profileIds } : {}
    );

    const resumeImportedChannelEnrichment = (options = {}) => sendImportedChannelEnrichmentCommand(
    ...
    const pauseImportedChannelEnrichment = () => sendImportedChannelEnrichmentCommand(
        'FilterTube_PauseImportedChannelEnrichment'
    );

    const explicitResumeImportedChannelEnrichment = () => resumeImportedChannelEnrichment({ unpause: true });

    const getImportedChannelEnrichmentStatus = () => sendImportedChannelEnrichmentCommand(
    ...
    function queueChannelForEnrichment(channel, profile = 'main') {
        // Every imported source is delegated to the persisted background
        // queue. Keeping these rows out of this ten-item session queue
        // prevents a large CSV/TXT/JSON import from using ordinary startup
        // pacing or waking this page-owned enrichment loop.
    ...
    return {
        ...
        pauseImportedChannelEnrichment,
        explicitResumeImportedChannelEnrichment,
