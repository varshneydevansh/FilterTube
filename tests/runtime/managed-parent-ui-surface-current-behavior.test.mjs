...
test('managed parent UI surface docs and runtime binding are linked', () => {
  ...
  assert.match(source, /if \(!hasReplicaUpdateTarget && !hasSourceAckTarget\) return Promise\.resolve\(null\)/);
  ...
  assert.doesNotMatch(source, /await runNanahManagedOpenSync\(\{ reason: 'profile_switch' \}\)/);
  ...
  assert.match(tabViewHtml, /Choose profile/);
  ...
  assert.match(tabViewHtml, /<strong id="ftNanahModeTitle" class="nanah-sync-mode-spotlight__title">Protected device<\/strong>/);
  assert.match(tabViewHtml, /send parent-approved rules, time limits, and viewing access to a protected profile or family device/);
  assert.match(tabViewHtml, /choose exactly which protected profile on the other device should receive this update/);
  assert.match(tabViewHtml, /Pick the protected profile in <strong>Profile to update<\/strong>/);
  assert.match(tabViewHtml, /Use this on the protected profile or family device that should accept matching parent updates/);
  assert.match(tabViewHtml, /The protected profile can watch within those rules/);
  ...
  assert.match(tabViewHtml, /More options/);
  ...
  assert.match(tabViewHtml, /class="rule-list-format-disclosure" open/);
  assert.match(tabViewHtml, /See examples before import/);
  ...
  assert.match(tabViewHtml, /Format examples parents can compare against/);
  assert.match(tabViewHtml, /Pick the file shape that looks closest to what you have/);
  assert.match(tabViewHtml, /rule-list-format-trust-strip/);
  assert.match(tabViewHtml, /Preview parsed rows/);
  assert.match(tabViewHtml, /Choose Main, Kids, or both/);
  assert.match(tabViewHtml, /Import only after parent approval/);
  assert.match(tabViewHtml, /rule-list-format-showcase__header/);
  assert.match(tabViewHtml, /rule-list-format-switcher/);
  assert.match(tabViewHtml, /id="ftRuleListFormatCsvPreview"[\s\S]*checked/);
  assert.match(tabViewHtml, /id="ftRuleListFormatJsonPreview"/);
  assert.match(tabViewHtml, /id="ftRuleListFormatTxtPreview"/);
  assert.match(tabViewHtml, /id="ftRuleListFormatUrlPreview"/);
  assert.match(tabViewHtml, /id="ftRuleListFormatBlockTubePreview"/);
  assert.match(tabViewHtml, /Choose a rule list format preview/);
  assert.match(tabViewHtml, />\s*Sheet columns\s*</);
  assert.match(tabViewHtml, />\s*Channels \+ keywords\s*</);
  assert.match(tabViewHtml, />\s*One row each\s*</);
  assert.match(tabViewHtml, />\s*Raw public list\s*</);
  ...
  assert.match(tabViewHtml, /rule-list-preview-sheet__formula/);
  ...
  assert.match(tabViewHtml, /channel: @SomeChannel/);
  assert.match(tabViewHtml, /Public URL preview/);
  assert.match(tabViewHtml, /Load a public list/);
  assert.match(tabViewHtml, /BlockTube export preview/);
  assert.match(tabViewHtml, /blocktube-export\.json/);
  assert.match(tabViewHtml, /filterData\.channelId/);
  assert.match(tabViewHtml, /filtertube-list\.json/);
  assert.match(tabViewHtml, /rule-list-preview-feedback/);
  assert.match(tabViewHtml, /Skipped rows/);
  assert.match(tabViewHtml, /Row 9 missing a channel ID or keyword/);
  assert.match(tabViewHtml, /Exact parser rules/);
  assert.match(tabViewHtml, /invalid rows are skipped with row numbers and reasons in preview/);
...
test('managed command-center spec pins parent workflow without making UI authority', () => {
  ...
  assert.match(plan, /the first-read panel now says `Family device updates`, `Choose profile`, and\s+`Connect the other device`/);
  assert.match(plan, /2026-06-20 Family Device Updates first-read simplification slice/);
