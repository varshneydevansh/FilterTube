...
const docPath = 'docs/audit/FILTERTUBE_JSON_FIRST_HIDE_VIDEO_DESCRIPTION_BOUNDARY_CURRENT_BEHAVIOR_2026-05-22.md';

const authoritySymbols = [
  'jsonFirstHideVideoDescriptionContract',
  ...
  'jsonFirstVideoDescriptionFixtureProvenance',
  'jsonFirstVideoDescriptionMetricArtifact'
];
...
function run(payload, overrides = {}, options = {}) {
  ...
  return plain(engine.processData(payload, settings(overrides), options.dataName || 'hide-video-description-boundary-fixture'));
}
...
function sourceBlocks() {
  ...
  return {
    ...
    domDescriptionCssRules: sliceBetween(
      domFallback,
...
test('JSON-first hideVideoDescription boundary audit is audit-only and source pinned', () => {
  const doc = read(docPath);

  assert.match(doc, /Status: audit-only current-behavior proof slice/);
  assert.match(doc, /Runtime behavior is unchanged/);
  assert.match(doc, /not an implementation patch, optimization patch, video-description/);
  assert.match(doc, /hideVideoDescription boundary source files: 6/);
  assert.match(doc, /runtime hideVideoDescription fixtures: 7/);
...
test('hideVideoDescription source counts remain pinned', () => {
  const doc = read(docPath);
  ...
  assert.equal(lineCount(blocks.domDescriptionCssRules), 8);
  assert.equal(Buffer.byteLength(blocks.domDescriptionCssRules), 291);
  ...
  assert.equal(lineCount(blocks.sharedBuildCompiledSettings), 62);
  assert.equal(Buffer.byteLength(blocks.sharedBuildCompiledSettings), 2314);
  assert.equal(lineCount(blocks.bridgeRefreshKeys), 44);
  assert.equal(Buffer.byteLength(blocks.bridgeRefreshKeys), 1263);

  assert.equal(countLiteral(blocks.filterLogic, 'hideVideoDescription'), 0);
  assert.equal(countLiteral(blocks.seed, 'hideVideoDescription'), 0);
  assert.equal(countLiteral(blocks.domFallback, 'hideVideoDescription'), 2);
  assert.equal(countLiteral(blocks.background, 'hideVideoDescription'), 12);
  assert.equal(countLiteral(blocks.settingsShared, 'hideVideoDescription'), 23);
  assert.equal(countLiteral(blocks.bridgeSettings, 'hideVideoDescription'), 1);
  ...
  assert.equal(countLiteral(blocks.domDescriptionCssRules, 'hideInfoMaster'), 1);
  assert.equal(countLiteral(blocks.domDescriptionCssRules, '#description.ytd-watch-metadata'), 1);
  assert.equal(countLiteral(blocks.domDescriptionCssRules, 'ytd-expander.ytd-video-secondary-info-renderer'), 1);
  ...
  assert.match(doc, /DOM fallback video-description CSS rules block lines: 8/);
  assert.match(doc, /DOM fallback video-description CSS rules block bytes: 291/);
  ...
  assert.match(doc, /settings_shared build compiled settings block lines: 62/);
  assert.match(doc, /settings_shared build compiled settings block bytes: 2314/);
  assert.match(doc, /filter_logic total hideVideoDescription tokens: 0/);
  assert.match(doc, /seed total hideVideoDescription tokens: 0/);
  assert.match(doc, /DOM fallback total hideVideoDescription tokens: 2/);
  assert.match(doc, /background total hideVideoDescription tokens: 12/);
  assert.match(doc, /settings_shared total hideVideoDescription tokens: 23/);
  assert.match(doc, /bridge_settings total hideVideoDescription token: 1/);
  ...
  assert.match(doc, /DOM fallback video-description CSS block hideInfoMaster tokens: 1/);
});

test('hideVideoDescription does not remove JSON watch metadata rows in filter_logic', () => {
  const payload = watchMetadataPayload();

  assert.deepEqual(run(payload, { hideVideoDescription: true }), plain(payload));
});

test('ordinary keyword rules can remove a watch metadata row while hideVideoDescription remains DOM-only', () => {
  const payload = watchMetadataPayload([
    videoPrimary('Block me description-adjacent title'),
...
test('whitelist mode preserves JSON video-info scaffolding even when hideVideoDescription is enabled', () => {
  const payload = watchMetadataPayload();
...
test('watch next hideVideoDescription alone bypasses JSON engine work through the no-active-JSON gate', async () => {
  const payload = watchMetadataPayload();
  const runtime = await loadSeedRuntime({
    pathname: '/watch',
    payload
  });
  runtime.window.filterTube.updateSettings(settings({ hideVideoDescription: true }));
...
test('source proof pins DOM-owned video-description hiding whitelist behavior refresh omission and missing authority', () => {
  const doc = read(docPath);
  const blocks = sourceBlocks();
  const runtimeSource = productRuntimeSource();

  assert.doesNotMatch(blocks.filterLogic, /hideVideoDescription/);
  assert.doesNotMatch(blocks.seed, /hideVideoDescription/);
  ...
  assert.doesNotMatch(blocks.seedActiveJsonRules, /hideVideoDescription/);
  assert.match(blocks.domVideoInfoModeDeclarations, /const hideInfoMaster = \(listMode !== 'whitelist'\) && !!settings\.hideVideoInfo/);
  assert.match(blocks.domDescriptionCssRules, /listMode !== 'whitelist'/);
  assert.match(blocks.domDescriptionCssRules, /hideInfoMaster \|\| settings\.hideVideoDescription/);
  assert.match(blocks.domDescriptionCssRules, /#description\.ytd-watch-metadata/);
  assert.match(blocks.domDescriptionCssRules, /ytd-expander\.ytd-video-secondary-info-renderer/);
  assert.match(blocks.domActiveBooleanKeys, /'hideVideoDescription'/);
  assert.match(blocks.bgStorageReadKeys, /'hideVideoDescription'/);
  assert.match(blocks.bgBooleanPassThrough, /compiledSettings\.hideVideoDescription = boolFromV4\('hideVideoDescription', items\.hideVideoDescription \|\| false\)/);
  assert.doesNotMatch(blocks.bgRefreshKeys, /'hideVideoDescription'/);
  assert.match(blocks.sharedSettingsKeys, /'hideVideoDescription'/);
  assert.match(blocks.sharedBuildCompiledSettings, /hideVideoDescription: !!hideVideoDescription/);
  assert.match(blocks.bridgeRefreshKeys, /'hideVideoDescription'/);
  assert.match(doc, /Background storage-change invalidation does not include `hideVideoDescription` today/);
  assert.match(doc, /Seed JSON active-work detection does not include `hideVideoDescription`/);
  assert.match(doc, /Watch video-description JSON rows pass through unchanged when only\s+`hideVideoDescription` is enabled/);
  assert.match(doc, /`\/youtubei\/v1\/next` now bypasses `processData` with only\s+`hideVideoDescription` enabled/);
