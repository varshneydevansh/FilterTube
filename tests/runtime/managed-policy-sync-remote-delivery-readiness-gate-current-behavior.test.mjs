...
test('managed remote delivery readiness gate is docs-backed and linked to current proof family', () => {
  ...
  assert.match(doc, /Remote policy authority, validation, local apply, action history,\s+provider-gated mailbox intake, and provider-gated local-network candidate intake\s+are present/);
  assert.match(doc, /Complete remote delivery is still blocked on transport proof/);
