...
test('DOM lifecycle and runtime observer docs carry the method proof gap blocker', () => {
  for (const docPath of domLifecycleReadinessDocs) {
    ...
    assert.match(doc, /method semantic proof gap lexical callables covered: 5469/, `${docPath} missing callable count`);
    assert.match(doc, /files with complete per-callable semantic proof: 0/, `${docPath} missing complete proof count`);
    assert.match(doc, /lexical callables requiring semantic proof before behavior changes: 5469/, `${docPath} missing required proof count`);
