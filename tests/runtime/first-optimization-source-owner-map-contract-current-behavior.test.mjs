...
test('source owner map contract is linked from audit ledgers and upstream gates', () => {
  ...
  assert.match(runtimeResults, /First optimization source owner map contract addendum:[\s\S]*63 method semantic proof gap files covered/);
  assert.match(activeGoal, /First optimization source owner map contract addendum:[\s\S]*63 method semantic proof gap files covered/);
  assert.match(objectiveLedger, /First optimization source owner map contract addendum:[\s\S]*63 method semantic proof gap files covered/);
  assert.match(trackedIndex, /First optimization source owner map contract addendum:[\s\S]*63 method semantic proof gap files covered/);
});
