...
test('metric foundation contract coverage rows counts and field names stay pinned', () => {
  ...
  assert.match(doc, /current generated runtime test declarations: 4731/);
  ...
  assert.equal(sourceTopLevelRuntimeTestDeclarationsFromIndex(), 4731);
...
test('metric foundation contract coverage gate is linked from audit ledgers and upstream gates', () => {
  ...
  assert.match(objectiveLedger, /First optimization metric foundation contract coverage gate addendum:[\s\S]*69 method semantic proof gap files covered/);
  assert.match(trackedIndex, /First optimization metric foundation contract coverage gate addendum:[\s\S]*69 method semantic proof gap files covered/);
});
