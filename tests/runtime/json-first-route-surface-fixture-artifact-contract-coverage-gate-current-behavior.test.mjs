...
test('JSON-first route/surface fixture artifact contract coverage gate is linked from ledgers and adjacent gates', () => {
  ...
  assert.match(read(sourceDocs.runtimeResults), /JSON-first route\/surface fixture artifact contract coverage gate addendum:[\s\S]*69\s+method\s+semantic\s+proof\s+gap\s+files covered/);
  assert.match(read(ledgerDocs.objectiveLedger), /JSON-first route\/surface fixture artifact contract coverage gate addendum[\s\S]*69\s+method\s+semantic\s+proof\s+gap\s+files covered/);
  assert.match(read(ledgerDocs.activeGoal), /JSON-first route\/surface fixture artifact contract coverage gate addendum[\s\S]*69\s+method\s+semantic\s+proof\s+gap\s+files covered/);
  assert.match(read(ledgerDocs.trackedIndex), /JSON-first route\/surface fixture artifact contract coverage gate addendum[\s\S]*69\s+method\s+semantic\s+proof\s+gap\s+files covered/);
});
