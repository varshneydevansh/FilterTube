...
test('whitelist optimization readiness gap matrix is audit-only and source-backed', () => {
  ...
  for (const whitelistDocPath of whitelistFamilyDocs) {
    ...
    assert.match(whitelistDoc, /method semantic proof gap lexical callables covered: 5827/, `${whitelistDocPath} missing callable count`);
    ...
    assert.match(whitelistDoc, /lexical callables requiring semantic proof before behavior changes: 5827/, `${whitelistDocPath} missing required proof count`);
