...
function assertCurrentLocalDistPackageSnapshot(doc) {
  ...
  const expectedBrowserStats = new Map([
    ['chrome', {
      ...
      zipBytes: 8728281,
      zipSha256: 'a14599fc1726507dbdfa049b9640f218c8538f367b6120b065c35fb7287628fb'
    }],
    ['firefox', {
      manifestBytes: 2603,
      ...
      webAccessibleResourceRefs: 5,
      zipBytes: 8728340,
      zipSha256: '4e64521b8ed5b4385a41896864794d1dae0d31f73b27f8d30973add7cbe00cc4'
    }],
    ['opera', {
      manifestBytes: 2518,
      ...
      zipBytes: 8728283,
      zipSha256: '475ae17a9b4c9f2c71a4b80b6e120f612d511a473bb888feeedbedb4459752db'
    }]
  ]);
  ...
  assert.match(doc, /browser staged files per directory: 61/);
  assert.match(doc, /dist zip artifacts: 3/);
  assert.match(doc, /total dist files including zips: 186/);
  assert.match(doc, /source-backed staged files per browser excluding manifest: 60/);
  assert.match(doc, /byte-identical source-backed staged files per browser excluding manifest: 60/);
  ...
  assert.equal(stats.distFiles.length, 186);
  ...
  for (const browser of distBrowserTargets) {
    ...
    assert.equal(actual.stagedFiles, 61);
    assert.equal(actual.version, '3.3.2');
    assert.equal(actual.sourceBackedFiles, 60);
    assert.equal(actual.byteIdenticalSourceBackedFiles, 60);
    ...
    assert.ok(
      doc.includes(`| \`${browser}\` | 61 | ${expected.manifestBytes} | \`${expected.manifestSha256}\` | \`3.3.2\` | ${expected.contentScriptEntries} | ${expected.contentScriptJsRefs} | ${expected.webAccessibleResourceRefs} | ${expected.zipBytes} | \`${expected.zipSha256}\` |`),
...
test('P0 release package audit documents blocked verdict and all named gates', () => {
  ...
  assert.match(methodGap, /repo-wide lexical callables: 6045/);
  assert.match(methodGap, /files with lexical accounting: 71/);
  assert.match(methodGap, /files with complete per-callable semantic proof: 0/);
  assert.match(methodGap, /lexical callables requiring semantic proof before behavior changes: 6045/);
  ...
  for (const familyDocPath of releasePackageFamilyDocs) {
    ...
    assert.match(familyDoc, /method semantic proof gap lexical callables covered: 6045/);
    assert.match(familyDoc, /files with complete per-callable semantic proof: 0/);
    assert.match(familyDoc, /lexical callables requiring semantic proof before behavior changes: 6045/);
