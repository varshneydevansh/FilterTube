...
const docPath = 'docs/audit/FILTERTUBE_JSON_FIRST_ROUTE_SURFACE_FIXTURE_ARTIFACT_PATH_BOUNDARY_CURRENT_BEHAVIOR_2026-05-24.md';
const runtimeTestPath = 'tests/runtime/json-first-route-surface-fixture-artifact-path-boundary-current-behavior.test.mjs';
...
const expectedRows = [
  'FT-JSON-FIXTURE-PATH-00-root',
  'FT-JSON-FIXTURE-PATH-01-manifest',
  'FT-JSON-FIXTURE-PATH-02-fixture-sample',
  'FT-JSON-FIXTURE-PATH-03-provenance',
  'FT-JSON-FIXTURE-PATH-04-parity-report',
  'FT-JSON-FIXTURE-PATH-05-verification-output'
];
...
test('JSON-first route/surface fixture artifact path boundary is audit-only and source-backed', () => {
  const doc = read(docPath);

  assert.match(doc, /Status: audit-only current-behavior JSON-first route\/surface fixture artifact\s+path boundary/);
  ...
  assert.match(doc, /Implementation-ready route\/surface fixture artifact path rows: 0/);
  assert.match(doc, /not completion proof for JSON-first route\/surface fixture artifact path authority/);
...
test('JSON-first route/surface fixture artifact path rows and counts stay pinned', () => {
  const doc = read(docPath);
  const rows = [...doc.matchAll(/^\| `(FT-JSON-FIXTURE-PATH-[^`]+)` \|/gm)].map((row) => row[1]);
...
test('reserved JSON-first route/surface fixture artifact paths are named but absent', () => {
  const doc = read(docPath);
...
test('JSON-first route/surface fixture artifact path boundary is backed by current packet and provenance gates', () => {
  ...
  assert.match(fixturePacketContract, /committed route\/surface fixture packet files: 0/);
...
test('JSON-first route/surface fixture artifact path symbols are absent from product source', () => {
  const doc = read(docPath);
...
test('JSON-first route/surface fixture artifact path boundary is linked from ledgers and adjacent gates', () => {
