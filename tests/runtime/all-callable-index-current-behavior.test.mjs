...
const expectedFamilyTotals = new Map([
  ...
  ['UI/settings runtime', { files: 12, count: 1832 }],
...
test('audit meta index docs carry the method proof gap blocker', () => {
  ...
  for (const docPath of auditMetaReadinessDocs) {
    ...
    assert.ok(doc.includes('method semantic proof gap lexical callables covered: 6035'), `${docPath} must pin callable gap count`);
...
test('all-callable index counts match current lexical callable source', () => {
  ...
  assert.equal(total, 6035);
  assert.match(indexDoc, /repo-wide lexical callables: 6035/);
});
...
test('all-callable index documents semantic method proof boundary', () => {
  ...
  assert.match(indexDoc, /another 2,865/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables: 6035/);
  ...
  assert.match(methodSemanticGapDoc, /lexical callables requiring semantic proof before behavior changes: 6035/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables still requiring semantic proof: 6035/);
