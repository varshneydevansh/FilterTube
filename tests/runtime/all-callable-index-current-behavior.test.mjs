...
const expectedFamilyTotals = new Map([
  ['Hot page/background runtime', { files: 9, count: 3121 }],
  ['Content helper runtime', { files: 9, count: 348 }],
  ['UI/settings runtime', { files: 10, count: 1582 }],
...
test('audit meta index docs carry the method proof gap blocker', () => {
  ...
  for (const docPath of auditMetaReadinessDocs) {
    ...
    assert.ok(doc.includes('method semantic proof gap lexical callables covered: 5736'), `${docPath} must pin callable gap count`);
...
test('all-callable index counts match current lexical callable source', () => {
  ...
  assert.equal(total, 5736);
  assert.match(indexDoc, /repo-wide lexical callables: 5736/);
});
...
test('all-callable index documents semantic method proof boundary', () => {
  ...
  assert.match(indexDoc, /Hot runtime has 3,121 lexical callables/);
  assert.match(indexDoc, /another 2,615/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables: 5736/);
  ...
  assert.match(methodSemanticGapDoc, /lexical callables requiring semantic proof before behavior changes: 5736/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables still requiring semantic proof: 5736/);
  ...
  assert.match(releaseFixStatusDoc, /current method semantic proof\s+gap index, which records `5,736` lexical callables/);
  ...
  assert.match(releaseRegressionDoc, /callable proof gap ledgers reconciled to 5,736 lexical callables/);
