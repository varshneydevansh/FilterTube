...
const expectedFamilyTotals = new Map([
  ...
  ['UI/settings runtime', { files: 10, count: 1676 }],
...
test('audit meta index docs carry the method proof gap blocker', () => {
  ...
  for (const docPath of auditMetaReadinessDocs) {
    ...
    assert.ok(doc.includes('method semantic proof gap lexical callables covered: 5875'), `${docPath} must pin callable gap count`);
...
test('all-callable index counts match current lexical callable source', () => {
  ...
  assert.equal(total, 5875);
  assert.match(indexDoc, /repo-wide lexical callables: 5875/);
});
...
test('all-callable index documents semantic method proof boundary', () => {
  ...
  assert.match(indexDoc, /another 2,709/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables: 5875/);
  ...
  assert.match(methodSemanticGapDoc, /lexical callables requiring semantic proof before behavior changes: 5875/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables still requiring semantic proof: 5875/);
  ...
  assert.match(releaseFixStatusDoc, /current method semantic proof\s+gap index, which records `5,875` lexical callables/);
  ...
  assert.match(releaseRegressionDoc, /callable proof gap ledgers reconciled to 5,875 lexical callables/);
