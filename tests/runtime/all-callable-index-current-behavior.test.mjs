...
test('audit meta index docs carry the method proof gap blocker', () => {
  ...
  for (const docPath of auditMetaReadinessDocs) {
    ...
    assert.ok(doc.includes('method semantic proof gap lexical callables covered: 6270'), `${docPath} must pin callable gap count`);
...
test('all-callable index accounts for every tracked JS JSX and MJS source file', () => {
  ...
  assert.match(indexDoc, /tracked JS\/JSX\/MJS files: 73/);
...
test('all-callable index counts match current lexical callable source', () => {
  ...
  assert.equal(total, 6270);
  assert.match(indexDoc, /repo-wide lexical callables: 6270/);
});
...
test('all-callable index documents semantic method proof boundary', () => {
  ...
  assert.match(indexDoc, /Hot runtime has 3,173 lexical callables/);
  assert.match(indexDoc, /another 3,097/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables: 6270/);
  ...
  assert.match(methodSemanticGapDoc, /lexical callables requiring semantic proof before behavior changes: 6270/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables still requiring semantic proof: 6270/);
