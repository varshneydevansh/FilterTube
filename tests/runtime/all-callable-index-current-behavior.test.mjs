...
const expectedFamilyTotals = new Map([
  ...
  ['UI/settings runtime', { files: 10, count: 1598 }],
...
test('all-callable index counts match current lexical callable source', () => {
  ...
  assert.equal(total, 5797);
  assert.match(indexDoc, /repo-wide lexical callables: 5797/);
});
...
test('all-callable index documents semantic method proof boundary', () => {
  ...
  assert.match(indexDoc, /another 2,631/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables: 5797/);
  ...
  assert.match(methodSemanticGapDoc, /lexical callables requiring semantic proof before behavior changes: 5797/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables still requiring semantic proof: 5797/);
  ...
  assert.match(releaseFixStatusDoc, /current method semantic proof\s+gap index, which records `5,797` lexical callables/);
  ...
  assert.match(releaseRegressionDoc, /callable proof gap ledgers reconciled to 5,797 lexical callables/);
