...
const expectedFamilyTotals = new Map([
  ...
  ['UI/settings runtime', { files: 14, count: 2023 }],
...
function assertMethodSemanticConvergenceBoundary(doc) {
  ...
  for (const phrase of [
    ...
    '6,231 lexical callables',
    '3,173 hot page/background runtime',
    '3,058 UI, helper, generated, vendor, build, audit-lane, website',
...
test('audit meta index docs carry the method proof gap blocker', () => {
  for (const phrase of [
    ...
    'repo-wide lexical callables: 6231',
    'files with complete per-callable semantic proof: 0',
    'lexical callables requiring semantic proof before behavior changes: 6231',
  ...
  for (const docPath of auditMetaReadinessDocs) {
    ...
    assert.ok(doc.includes('method semantic proof gap lexical callables covered: 6231'), `${docPath} must pin callable gap count`);
...
test('all-callable index counts match current lexical callable source', () => {
  ...
  assert.equal(total, 6231);
  assert.match(indexDoc, /repo-wide lexical callables: 6231/);
});
...
test('all-callable index documents semantic method proof boundary', () => {
  ...
  assert.match(indexDoc, /another 3,058/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables: 6231/);
  ...
  assert.match(methodSemanticGapDoc, /lexical callables requiring semantic proof before behavior changes: 6231/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables still requiring semantic proof: 6231/);
