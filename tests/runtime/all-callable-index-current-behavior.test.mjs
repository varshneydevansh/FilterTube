...
function assertMethodSemanticConvergenceBoundary(doc) {
  ...
  for (const phrase of [
    ...
    '6,228 lexical callables',
    '3,173 hot page/background runtime',
    '3,055 UI, helper, generated, vendor, build, audit-lane, website',
...
test('audit meta index docs carry the method proof gap blocker', () => {
  ...
  for (const docPath of auditMetaReadinessDocs) {
    ...
    assert.ok(doc.includes('method semantic proof gap lexical callables covered: 6228'), `${docPath} must pin callable gap count`);
...
test('all-callable index counts match current lexical callable source', () => {
  ...
  assert.equal(total, 6228);
  assert.match(indexDoc, /repo-wide lexical callables: 6228/);
});
...
test('all-callable index documents semantic method proof boundary', () => {
  ...
  assert.match(indexDoc, /another 3,055/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables: 6228/);
  ...
  assert.match(methodSemanticGapDoc, /lexical callables requiring semantic proof before behavior changes: 6228/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables still requiring semantic proof: 6228/);
