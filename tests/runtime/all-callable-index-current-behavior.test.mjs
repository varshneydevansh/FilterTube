...
const expectedFamilyTotals = new Map([
  ['Hot page/background runtime', { files: 9, count: 3177 }],
  ['Content helper runtime', { files: 9, count: 348 }],
  ['UI/settings runtime', { files: 14, count: 2094 }],
...
test('audit meta index docs carry the method proof gap blocker', () => {
  ...
  for (const docPath of auditMetaReadinessDocs) {
    ...
    assert.ok(doc.includes('method semantic proof gap lexical callables covered: 6306'), `${docPath} must pin callable gap count`);
...
test('all-callable index counts match current lexical callable source', () => {
  ...
  assert.equal(total, 6306);
  assert.match(indexDoc, /repo-wide lexical callables: 6306/);
});
...
test('all-callable index documents semantic method proof boundary', () => {
  ...
  assert.match(indexDoc, /Hot runtime has 3,177 lexical callables/);
  assert.match(indexDoc, /another 3,129/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables: 6306/);
  ...
  assert.match(methodSemanticGapDoc, /lexical callables requiring semantic proof before behavior changes: 6306/);
  ...
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables still requiring semantic proof: 6306/);
