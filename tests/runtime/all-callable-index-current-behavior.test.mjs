...
const expectedFamilyTotals = new Map([
  ['Hot page/background runtime', { files: 9, count: 3170 }],
  ['Content helper runtime', { files: 9, count: 348 }],
  ['UI/settings runtime', { files: 11, count: 1743 }],
...
test('audit meta index docs carry the method proof gap blocker', () => {
  ...
  for (const docPath of auditMetaReadinessDocs) {
    ...
    assert.ok(doc.includes('method semantic proof gap lexical callables covered: 5946'), `${docPath} must pin callable gap count`);
...
test('all-callable index accounts for every tracked JS JSX and MJS source file', () => {
  ...
  assert.match(indexDoc, /tracked JS\/JSX\/MJS files: 70/);
...
test('all-callable index counts match current lexical callable source', () => {
  ...
  assert.equal(total, 5946);
  assert.match(indexDoc, /repo-wide lexical callables: 5946/);
});
...
test('all-callable index documents semantic method proof boundary', () => {
  ...
  assert.match(indexDoc, /Hot runtime has 3,170 lexical callables/);
  assert.match(indexDoc, /another 2,776/);
  ...
  assert.match(methodSemanticGapDoc, /tracked JS\/JSX\/MJS files: 70/);
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables: 5946/);
  assert.match(methodSemanticGapDoc, /files with lexical accounting: 70/);
  assert.match(methodSemanticGapDoc, /files with complete per-callable semantic proof: 0/);
  assert.match(methodSemanticGapDoc, /lexical callables requiring semantic proof before behavior changes: 5946/);
  ...
  assert.match(methodSemanticGapDoc, /tracked JS\/JSX\/MJS files still covered: 70/);
  assert.match(methodSemanticGapDoc, /repo-wide lexical callables still requiring semantic proof: 5946/);
  ...
  assert.match(releaseFixStatusDoc, /current method semantic proof\s+gap index, which records `5,946` lexical callables/);
  ...
  assert.match(releaseRegressionDoc, /callable proof gap ledgers reconciled to 5,946 lexical callables/);
