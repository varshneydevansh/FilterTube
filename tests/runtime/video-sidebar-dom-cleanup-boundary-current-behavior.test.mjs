...
const docPath = 'docs/audit/FILTERTUBE_VIDEO_SIDEBAR_DOM_CLEANUP_BOUNDARY_CURRENT_BEHAVIOR_2026-05-22.md';
...
function sourceBlocks() {
  ...
  return {
    ...
    videoSidebarCssBlock: sliceBetween(
      source,
...
test('video sidebar DOM cleanup boundary audit is audit-only and source pinned', () => {
  ...
  assert.match(doc, /video-sidebar DOM cleanup boundary source files: 1/);
  assert.match(doc, /video-sidebar DOM cleanup source\/effect blocks: 6/);
  assert.match(doc, /runtime video-sidebar DOM cleanup fixtures: 6/);

  assert.ok(doc.includes(`| \`js/content/dom_fallback.js\` | 5,030 | 235,555 | \`${sha256('js/content/dom_fallback.js')}\` |`));
});

test('video sidebar DOM cleanup source counts remain pinned', () => {
  const doc = read(docPath);
  const {
    source,
    ensureBlock,
    videoSidebarCssBlock,
    activeBlock,
  ...
  assert.equal(lineCount(videoSidebarCssBlock), 7);
  assert.equal(Buffer.byteLength(videoSidebarCssBlock), 172);
  ...
  assert.equal(countLiteral(videoSidebarCssBlock, 'rules.push'), 1);
  assert.equal(countLiteral(videoSidebarCssBlock, 'display: none !important'), 1);
  assert.equal(countLiteral(videoSidebarCssBlock, '#secondary.ytd-watch-flexy'), 1);
  assert.equal(countLiteral(ensureBlock, 'hideVideoSidebar'), 1);
  ...
  assert.equal(countLiteral(activeBlock, "'hideVideoSidebar'"), 1);
  assert.equal(countLiteral(clearBlock, 'hideVideoSidebar'), 0);
  assert.equal(countLiteral(disabledCleanupBlock, 'hideVideoSidebar'), 0);
  assert.equal(countLiteral(clearBlock, 'data-filtertube-hidden-by-hide-video-sidebar'), 0);
  assert.equal(countLiteral(disabledCleanupBlock, 'data-filtertube-hidden-by-hide-video-sidebar'), 0);
  assert.equal(countLiteral(source, 'data-filtertube-hidden-by-hide-video-sidebar'), 0);
  assert.equal(countLiteral(source, 'hideVideoSidebar'), 2);
  ...
  assert.match(doc, /video-sidebar CSS block lines: 7/);
  assert.match(doc, /video-sidebar CSS block bytes: 172/);
  assert.match(doc, /video-sidebar CSS selector rows: 1/);
  assert.match(doc, /clear-stale cleanup video-sidebar marker references: 0/);
  assert.match(doc, /disabled cleanup video-sidebar marker references: 0/);
  assert.match(doc, /product runtime video-sidebar marker references: 0/);
});

test('video sidebar CSS branch writes secondary-column selector route attributes and open-app cleanup coupling', () => {
  const document = createDocument({ pathname: '/watch' });
  const sandbox = runEnsure({ hideVideoSidebar: true }, { document });
  ...
  assert.match(css, /#secondary\.ytd-watch-flexy/);
  assert.doesNotMatch(css, /#related/);
  assert.doesNotMatch(css, /ytd-live-chat-frame#chat/);
  assert.equal(countLiteral(css, 'display: none !important'), 2);
});

test('video sidebar style restore is shared style regeneration without feature-local marker cleanup', () => {
  const document = createDocument({ pathname: '/watch' });
  runEnsure({ hideVideoSidebar: true }, { document });
  const initialStyle = document.__elements.get('filtertube-content-controls-style');

  assert.match(initialStyle.textContent, /#secondary\.ytd-watch-flexy/);
  assert.equal(document.head.appended.length, 1);

  const sandbox = runEnsure({ hideVideoSidebar: false }, { document });
  const rewrittenStyle = document.__elements.get('filtertube-content-controls-style');

  assert.equal(rewrittenStyle, initialStyle);
  assert.equal(document.head.appended.length, 1);
  assert.doesNotMatch(rewrittenStyle.textContent, /#secondary\.ytd-watch-flexy/);
  assert.match(rewrittenStyle.textContent, /ytm-button-renderer a\[href\^="intent:\/\/"\]/);
  assert.equal(sandbox.__openAppCalls, 1);
});

test('video sidebar CSS remains enabled in whitelist mode and without has selector support', () => {
  const whitelistDocument = createDocument({ pathname: '/watch' });
  runEnsure({ hideVideoSidebar: true, listMode: 'whitelist' }, { document: whitelistDocument });
  const whitelistCss = styleText(whitelistDocument);

  assert.match(whitelistCss, /#secondary\.ytd-watch-flexy/);

  const noHasDocument = createDocument({ pathname: '/watch' });
  const sandbox = runEnsure(
    { hideVideoSidebar: true, hidePlaylistCards: true },
    { document: noHasDocument, supportsHasSelector: false }
  );
  const noHasCss = styleText(noHasDocument);

  assert.deepEqual(sandbox.__supportsQueries, ['selector(:has(*))']);
  assert.match(noHasCss, /#secondary\.ytd-watch-flexy/);
  assert.match(noHasCss, /yt-lockup-view-model:has\(yt-collections-stack\)/);
});

test('video sidebar DOM cleanup boundary records marker omissions and missing future authorities outside runtime source', () => {
  const doc = read(docPath);
  const { source, clearBlock, disabledCleanupBlock } = sourceBlocks();
  const runtime = productRuntimeSource();

  assert.equal(source.includes('data-filtertube-hidden-by-hide-video-sidebar'), false);
  assert.equal(clearBlock.includes('data-filtertube-hidden-by-hide-video-sidebar'), false);
  assert.equal(disabledCleanupBlock.includes('data-filtertube-hidden-by-hide-video-sidebar'), false);
  assert.match(doc, /style-owned rather than a JSON watch-next sidebar decision or marker-owned DOM decision/);
  assert.match(doc, /Restore is a full style regeneration effect, not a per-node marker restore/);
  assert.match(doc, /Video sidebar style refresh also triggers unrelated mobile app-shell cleanup work/);
