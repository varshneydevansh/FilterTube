...
test('first optimization candidate selection boundary is audit-only and source-backed', () => {
  ...
  for (const firstOptimizationDocPath of firstOptimizationMethodGapDocs) {
    ...
    assert.match(firstOptimizationDoc, /method semantic proof gap lexical callables covered: 5701/, `${firstOptimizationDocPath} missing callable count`);
    ...
    assert.match(firstOptimizationDoc, /lexical callables requiring semantic proof before behavior changes: 5701/, `${firstOptimizationDocPath} missing required proof count`);
