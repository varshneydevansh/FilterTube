...
const docPath = 'docs/audit/FILTERTUBE_LIVE_CHAT_DOM_CLEANUP_BOUNDARY_CURRENT_BEHAVIOR_2026-05-22.md';
...
test('live chat DOM cleanup boundary audit is audit-only and source pinned', () => {
  ...
  assert.match(doc, /live-chat DOM cleanup boundary source files: 1/);
...
test('live chat DOM cleanup source counts remain pinned', () => {
  ...
  assert.equal(lineCount(liveChatCssBlock), 8);
  assert.equal(Buffer.byteLength(liveChatCssBlock), 195);
  ...
  assert.equal(countLiteral(liveChatCssBlock, 'rules.push'), 1);
  assert.equal(countLiteral(liveChatCssBlock, 'display: none !important'), 1);
  assert.equal(countLiteral(liveChatCssBlock, 'ytd-live-chat-frame#chat'), 1);
  assert.equal(countLiteral(liveChatCssBlock, '#chat-container'), 1);
  assert.equal(countLiteral(ensureBlock, 'hideLiveChat'), 1);
  ...
  assert.equal(countLiteral(activeBlock, "'hideLiveChat'"), 1);
  assert.equal(countLiteral(clearBlock, 'hideLiveChat'), 0);
  assert.equal(countLiteral(disabledCleanupBlock, 'hideLiveChat'), 0);
  assert.equal(countLiteral(clearBlock, 'data-filtertube-hidden-by-hide-live-chat'), 0);
  assert.equal(countLiteral(disabledCleanupBlock, 'data-filtertube-hidden-by-hide-live-chat'), 0);
  assert.equal(countLiteral(source, 'data-filtertube-hidden-by-hide-live-chat'), 0);
  assert.equal(countLiteral(source, 'hideLiveChat'), 2);
  assert.equal(countLiteral(source, 'ytd-live-chat-frame#chat'), 1);
  assert.equal(countLiteral(source, '#chat-container'), 1);
  assert.equal(countLiteral(source, '#secondary.ytd-watch-flexy'), 1);
  assert.equal(countLiteral(source, '#related'), 1);

  assert.match(doc, /live-chat CSS block lines: 8/);
  assert.match(doc, /live-chat CSS block bytes: 195/);
  assert.match(doc, /live-chat CSS selector rows: 2/);
  assert.match(doc, /clear-stale cleanup live-chat marker references: 0/);
  assert.match(doc, /disabled cleanup live-chat marker references: 0/);
  assert.match(doc, /product runtime live-chat marker references: 0/);
});

test('live chat CSS branch writes chat selectors route attributes and open-app cleanup coupling', () => {
  const document = createDocument({ pathname: '/watch' });
  const sandbox = runEnsure({ hideLiveChat: true }, { document });
  ...
  assert.match(css, /ytd-live-chat-frame#chat/);
  assert.match(css, /#chat-container/);
...
test('live chat style restore is shared style regeneration without feature-local marker cleanup', () => {
  const document = createDocument({ pathname: '/watch' });
  runEnsure({ hideLiveChat: true }, { document });
  const initialStyle = document.__elements.get('filtertube-content-controls-style');

  assert.match(initialStyle.textContent, /ytd-live-chat-frame#chat/);
  assert.match(initialStyle.textContent, /#chat-container/);
  assert.equal(document.head.appended.length, 1);

  const sandbox = runEnsure({ hideLiveChat: false }, { document });
  const rewrittenStyle = document.__elements.get('filtertube-content-controls-style');

  assert.equal(rewrittenStyle, initialStyle);
  assert.equal(document.head.appended.length, 1);
  assert.doesNotMatch(rewrittenStyle.textContent, /ytd-live-chat-frame#chat/);
  assert.doesNotMatch(rewrittenStyle.textContent, /#chat-container/);
  assert.match(rewrittenStyle.textContent, /ytm-button-renderer a\[href\^="intent:\/\/"\]/);
  assert.equal(sandbox.__openAppCalls, 1);
});

test('live chat CSS remains enabled in whitelist mode and without has selector support', () => {
  const whitelistDocument = createDocument({ pathname: '/watch' });
  runEnsure({ hideLiveChat: true, listMode: 'whitelist' }, { document: whitelistDocument });
  const whitelistCss = styleText(whitelistDocument);

  assert.match(whitelistCss, /ytd-live-chat-frame#chat/);
  assert.match(whitelistCss, /#chat-container/);

  const noHasDocument = createDocument({ pathname: '/watch' });
  const sandbox = runEnsure(
    { hideLiveChat: true, hidePlaylistCards: true },
    { document: noHasDocument, supportsHasSelector: false }
  );
  const noHasCss = styleText(noHasDocument);

  assert.deepEqual(sandbox.__supportsQueries, ['selector(:has(*))']);
  assert.match(noHasCss, /ytd-live-chat-frame#chat/);
  assert.match(noHasCss, /#chat-container/);
  assert.match(noHasCss, /yt-lockup-view-model:has\(yt-collections-stack\)/);
});

test('live chat DOM cleanup boundary records marker omissions and missing future authorities outside runtime source', () => {
  const doc = read(docPath);
  const { source, clearBlock, disabledCleanupBlock } = sourceBlocks();
  const runtime = productRuntimeSource();

  assert.equal(source.includes('data-filtertube-hidden-by-hide-live-chat'), false);
  assert.equal(clearBlock.includes('data-filtertube-hidden-by-hide-live-chat'), false);
  assert.equal(disabledCleanupBlock.includes('data-filtertube-hidden-by-hide-live-chat'), false);
  assert.match(doc, /style-owned rather than a JSON watch-next engagement-panel decision or marker-owned DOM decision/);
  assert.match(doc, /Restore is a full style regeneration effect, not a per-node marker restore/);
  assert.match(doc, /Live chat style refresh also triggers unrelated mobile app-shell cleanup work/);
