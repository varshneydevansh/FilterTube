...
const docPath = 'docs/audit/FILTERTUBE_JSON_FIRST_ROUTE_SURFACE_FIXTURE_SAMPLE_CONTRACT_CURRENT_BEHAVIOR_2026-05-24.md';
const runtimeTestPath = 'tests/runtime/json-first-route-surface-fixture-sample-contract-current-behavior.test.mjs';
const samplePath = 'docs/audit/artifacts/json-first/route-surface-fixture-packet/fixture-sample.json';
...
const expectedRows = [
  'FT-JSON-FIXTURE-SAMPLE-00-sample-identity',
  'FT-JSON-FIXTURE-SAMPLE-01-route-surface-binding',
  'FT-JSON-FIXTURE-SAMPLE-02-mode-rule-state',
  'FT-JSON-FIXTURE-SAMPLE-03-source-fixture-paths',
  'FT-JSON-FIXTURE-SAMPLE-04-positive-hide',
  'FT-JSON-FIXTURE-SAMPLE-05-negative-sibling-visible',
  'FT-JSON-FIXTURE-SAMPLE-06-disabled-empty-no-rule',
  'FT-JSON-FIXTURE-SAMPLE-07-sparse-selected-continuation',
  'FT-JSON-FIXTURE-SAMPLE-08-parity-playback-restore',
  'FT-JSON-FIXTURE-SAMPLE-09-no-work-side-effect-budget',
  'FT-JSON-FIXTURE-SAMPLE-10-diagnostic-metric-privacy',
  'FT-JSON-FIXTURE-SAMPLE-11-verification-rollback'
];
...
const requiredFields = [
  ...
  'expectedHiddenChannels',
  'expectedHiddenRenderers',
  'negativeSiblingFixture',
  'expectedVisibleVideoIds',
  'expectedVisibleChannels',
  'unsupportedRendererBoundary',
  'disabledFixture',
  'emptyListFixture',
  'noRuleFixture',
...
test('JSON-first route/surface fixture sample contract is audit-only and source-backed', () => {
  const doc = read(docPath);

  assert.match(doc, /Status: audit-only current-behavior JSON-first route\/surface fixture sample\s+contract/);
  assert.match(doc, /Runtime behavior is unchanged/);
  assert.match(doc, /This is not an implementation patch,\s+optimization patch/);
  assert.match(doc, /Reserved route\/surface fixture sample path: docs\/audit\/artifacts\/json-first\/route-surface-fixture-packet\/fixture-sample\.json/);
  assert.match(doc, /Committed route\/surface fixture sample files: 0/);
  assert.match(doc, /Runtime JSON-first fixture sample approval exists: no/);
  assert.match(doc, /Runtime JSON-first fixture packet approval exists: no/);
  assert.match(doc, /Implementation-ready JSON-first fixture sample contract rows: 0/);
  assert.match(doc, /not completion proof for JSON-first route\/surface fixture sample authority/);
...
test('JSON-first route/surface fixture sample rows counts and fields stay pinned', () => {
  const doc = read(docPath);
  const rows = [...doc.matchAll(/^\| `(FT-JSON-FIXTURE-SAMPLE-[^`]+)` \|/gm)].map((row) => row[1]);

  assert.deepEqual(rows, expectedRows);
  assert.equal(rows.length, 12);
  assert.match(doc, /JSON-first route\/surface fixture sample contract rows: 12/);
  assert.match(doc, /reserved fixture sample paths covered: 1/);
  ...
  assert.match(doc, /runtime JSON-first fixture sample approvals: 0/);
  ...
  for (const field of requiredFields) {
    assert.ok(doc.includes(field), `missing sample field ${field}`);
  }
});

test('JSON-first route/surface fixture sample path is reserved but not committed yet', () => {
  const doc = read(docPath);
...
test('JSON-first route/surface fixture sample contract is backed by current NO-GO gates', () => {
...
test('JSON-first route/surface fixture sample decisions and authority symbols are absent from product source', () => {
  const doc = read(docPath);
...
test('JSON-first route/surface fixture sample contract is linked from ledgers and adjacent gates', () => {
  ...
  assert.match(read(sourceDocs.runtimeResults), /JSON-first route\/surface fixture sample contract addendum:[\s\S]*69\s+method\s+semantic\s+proof\s+gap\s+files covered/i);
  assert.match(read(ledgerDocs.objectiveLedger), /JSON-first route\/surface fixture sample contract addendum[\s\S]*69\s+method\s+semantic\s+proof\s+gap\s+files covered/i);
  assert.match(read(ledgerDocs.activeGoal), /JSON-first route\/surface fixture sample contract addendum[\s\S]*69\s+method\s+semantic\s+proof\s+gap\s+files covered/i);
  assert.match(read(ledgerDocs.trackedIndex), /JSON-first route\/surface fixture sample contract addendum[\s\S]*69\s+method\s+semantic\s+proof\s+gap\s+files covered/i);
});
