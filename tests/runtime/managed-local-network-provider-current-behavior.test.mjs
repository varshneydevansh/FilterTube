...
test('local-network provider hook is docs-backed and linked from managed parent plan', () => {
  ...
  assert.match(doc, /Built-in\s+same-network peer discovery and same-network transport remain absent/);
  assert.match(doc, /Browser\s+HTTPS Internet Pickup pull\/decrypt is handled by the separate configured\s+Internet Pickup client and does not grant local-network authority/);
  ...
  assert.match(doc, /A successful health response means only that the Home Pickup\s+service endpoint answered/);
  ...
  assert.match(doc, /runtime local-network provider health status persistence: present and redacted/);
  ...
  assert.match(doc, /Waiting for Home Pickup \(age\)/);
  ...
  assert.match(sourceAckDoc, /Source-side Internet Pickup\/Home Pickup delivery ack intake and\s+dashboard status are present/);
...
test('dashboard source wires provider-gated local-network discovery without YouTube hot-path primitives', () => {
  ...
  assert.match(source, /const NANAH_MANAGED_LOCAL_NETWORK_HEALTH_KEY = 'ftManagedLocalNetworkProviderHealthState'/);
  assert.match(source, /function writeNanahManagedLocalNetworkProviderHealthState\(state\)/);
  assert.match(source, /function checkNanahManagedLocalNetworkProviderHealth\(\{ reason = 'manual_check', silent = false \} = \{\}\)/);
  assert.match(source, /Check Pickup/);
  assert.match(source, /Last Home Pickup check passed/);
  assert.match(source, /Last Home Pickup check did not pass/);
  assert.match(source, /Home Pickup saved and reachable/);
  assert.match(source, /Home Pickup saved, but it did not answer the readiness check/);
  ...
  assert.match(source, /Home Pickup receipt recorded/);
  ...
  assert.match(source, /Waiting for Home Pickup/);
  assert.match(source, /\$\{accepted\} accepted, \$\{rejected\} rejected, \$\{ackFailed\} ack failed/);
  assert.match(source, /Home Pickup rejected/);
  ...
  assert.match(source, /Home Pickup/);
});
