...
const affectedCallableAnchorChecks = [
  ...
  ['js/background.js', 2065, 'function enqueueVideoChannelMapUpdate(videoId, channelId) {'],
  ['js/background.js', 2090, 'function enqueueVideoMetaMapUpdate(videoId, meta) {'],
  ['js/background.js', 2191, 'async function getCompiledSettings(sender = null, profileType = null, forceRefresh = false) {'],
...
test('whitelist/cache SPA metric packet gate is audit-only and source-backed', () => {
  ...
  assert.match(affectedDoc, /affected lexical callables requiring semantic proof: 2873/);
...
test('packet rows, live smoke rows, and diagrams remain explicit', () => {
  ...
  assert.match(methodGapDoc, /method semantic proof gap lexical callables covered: 5836/);
  assert.match(methodGapDoc, /files with complete per-callable semantic proof: 0/);
  assert.match(methodGapDoc, /lexical callables requiring semantic proof before behavior changes: 5836/);
...
test('metric packet gate preserves optimization and JSON-first approval absence', () => {
  ...
  for (const ledgerDoc of affectedCallableLedgerDocs) {
    ...
    assert.match(ledgerSource, /2873\s+affected\s+lexical\s+callables\s+requiring\s+semantic\s+proof/);
