...
function assertReleaseHotPathProofStackAddendum(source) {
  ...
  assert.match(jsonFirstImplementationAuthorityDoc, /\| `json_flow_renderer_rule_owner` \| `js\/filter_logic\.js:426-835` \|/);
  ...
  assert.match(source, /current runtime test files: 527/);
  assert.match(source, /current source top-level test declarations counted: 4667/);
  assert.match(source, /current full runtime proof for generated 4667 declaration set: GO/);
  ...
  assert.match(source, /classifies all 510 lifecycle instances into 460 install\/schedule\s+sites and 50 explicit teardown\/clear\/cancel sites/);
  ...
  assert.match(lifecycleDoc, /\| `install-or-schedule` \| `addEventListener`, `MutationObserver`, `IntersectionObserver`, `setInterval`, `setTimeout`, `requestAnimationFrame` \| 460 \|/);
  assert.match(lifecycleDoc, /\| `explicit-teardown` \| `removeEventListener`, `clearInterval`, `clearTimeout`, `cancelAnimationFrame` \| 50 \|/);
  assert.match(lifecycleDoc, /install-to-teardown ratio: 9\.2:1/);
  ...
  assert.match(source, /classifies all 288 current `addEventListener` installs into 232\s+omitted-option listeners, 23 boolean capture listeners, 30 object-option\s+listeners, 1 explicit bubble listener, and 2 generated expression\/identifier\s+option listeners/);
  ...
  assert.match(lifecycleDoc, /no-third-argument listener installs: 232/);
  ...
  assert.match(source, /classifies all 288 current `addEventListener` installs into 114\s+click listeners, 55 change listeners, 20 input listeners, 14 keydown listeners,\s+8 `DOMContentLoaded` listeners, 1 `ended` media listener, 72 other literal\s+event listeners, 4 non-literal event expressions, and 0 missing event\s+arguments/);
  ...
  assert.match(lifecycleDoc, /change listener installs: 55/);
  ...
  assert.match(source, /classifies all 288 current `addEventListener` installs into 203\s+local element targets, 17 optional local element targets, 39 document targets,\s+19 window targets, 8 vendor transport targets, and 2 generated shell targets/);
  ...
  assert.match(lifecycleDoc, /local element listener targets: 203/);
  assert.match(lifecycleDoc, /optional local element listener targets: 17/);
  assert.match(lifecycleDoc, /document listener targets: 39/);
  ...
  assert.match(source, /10 document click pairs, 7 document `DOMContentLoaded` pairs, 3\s+document keydown pairs, 4 document pointer\/mouse pairs, 4 window message pairs,\s+2 window route pairs, 9 window scroll\/resize\/orientation pairs, 1 window\s+storage\/visibility pair, 104 local click pairs, 68 local change\/input\/keydown\s+pairs, 8 vendor transport lifecycle pairs, and 2 generated shell nonliteral\s+pairs/);
  ...
  assert.match(source, /classifies all 17 current tracked/);
  ...
  assert.match(source, /classifies all 17 current tracked `\.observe\(\.\.\.\)` activation\s+calls by option shape: 9 `childList \+ subtree` observers, 1 `childList`-only\s+observer, 2 no-option visibility observers, and 5 attribute-filter observers/);
  assert.match(source, /2 style\/hidden attribute filters, 1 `aria-hidden`\s+attribute filter, 1 `disabled` attribute filter, 1 collaborator identity\s+attribute filter, 16 content-runtime observer observe option rows, and 1\s+extension UI\/background observer observe option row/);
  ...
  assert.match(lifecycleDoc, /observer observe no-options rows: 2/);
  ...
  assert.match(source, /classifies all 10 current tracked observer `\.disconnect\(\)` and\s+optional-chain `\.disconnect\?\.\(\)` invocations/);
  ...
  assert.match(lifecycleDoc, /local observer variable disconnect calls: 5/);
  ...
  assert.match(source, /joins all 17 current tracked `\.observe\(\.\.\.\)` activation rows\s+to 11 release rows: 10 `\.disconnect\(\.\.\.\)` or `\.disconnect\?\.\(\.\.\.\)` rows and 1\s+`\.unobserve\(\.\.\.\)` row/);
  assert.match(source, /current row-count delta is 6 observe rows over\s+release rows/);
  assert.match(source, /10 local `observer` observe rows, 2 local `obs` observe\s+rows, 5 exact named observer observe rows, 4 exact named observer observe rows\s+with release, 1 exact named observer observe row without release, 1\s+`prefetchObserver\.observe\(card\)` row without direct release/);
  assert.match(source, /content-runtime\s+observe\/release delta of 5, and an extension UI\/background observe\/release\s+delta of 1/);
  ...
  assert.match(lifecycleDoc, /observer disconnect release rows: 10/);
  ...
  assert.match(source, /compares all 17 observer constructor rows to all 17\s+`\.observe\(\.\.\.\)` activation rows by observer type/);
  assert.match(source, /15 `MutationObserver`\s+constructor rows, 2 `IntersectionObserver` constructor rows, 15 mutation\s+observer observe rows, and 2 intersection observer observe rows/);
  assert.match(source, /constructor-minus-observe delta is 0 overall, 0 for mutation\s+observer rows, 0 for intersection observer rows, 0 for content-runtime rows,\s+and 0 for extension UI\/background rows/);
  ...
  assert.match(lifecycleDoc, /MutationObserver constructor rows for type parity: 15/);
  assert.match(lifecycleDoc, /IntersectionObserver constructor rows for type parity: 2/);
  assert.match(lifecycleDoc, /observer observe rows for type parity: 17/);
  ...
  assert.match(source, /classifies all 17 current `new MutationObserver\(\.\.\.\)`\s+and `new IntersectionObserver\(\.\.\.\)` callback arguments into 17 inline arrow\s+callbacks, 0 identifier callbacks, and 0 missing callbacks/);
  assert.match(source, /9 observer callbacks with a `mutations` parameter, 2\s+observer callbacks with an `entries` parameter, 6 observer callbacks with no\s+parameter, 16 content-runtime observer constructor callbacks, and 1 extension\s+UI\/background observer constructor callback/);
  ...
  assert.match(lifecycleDoc, /observer callbacks with no parameter: 6/);
  ...
  assert.match(source, /classifies all 50 current explicit teardown rows into 9\s+`removeEventListener` rows, 34 `clearTimeout` rows, 4 `clearInterval` rows, 3\s+`cancelAnimationFrame` rows, 5 listener document targets, 2 listener window\s+targets, 2 generated shell listener targets, 12 local timeout id handles, 14\s+named timeout state handles, 8 property-held timeout handles, 2 engine-check\s+interval handles, 1 warmup interval handle, 1 dashboard rotation interval\s+handle, 2 profile dropdown frame handles, and 1 generic position frame handle/);
  ...
  assert.match(lifecycleDoc, /removeEventListener teardown rows: 9/);
  assert.match(lifecycleDoc, /clearTimeout teardown rows: 34/);
  assert.match(lifecycleDoc, /clearInterval teardown rows: 4/);
  assert.match(lifecycleDoc, /cancelAnimationFrame teardown rows: 3/);
  assert.match(lifecycleDoc, /listener document teardown targets: 5/);
  ...
  assert.match(source, /classifies all 288 current `addEventListener` callback arguments\s+into 252 inline arrow callbacks, 33 identifier callback references, 1 member\s+callback reference, 2 generated expression callbacks, 74 content-runtime\s+callbacks, 201 extension UI\/background callbacks, 2 generated-output\s+callbacks, 8 vendor-bundle callbacks, and 3 website-component callbacks/);
  ...
  assert.match(lifecycleDoc, /identifier listener callbacks: 33/);
  ...
  assert.match(lifecycleDoc, /website component listener callbacks: 3/);
  ...
  assert.match(lifecycleDoc, /removeEventListener teardown rows for parity: 9/);
  ...
  assert.match(lifecycleDoc, /exact option-shape listener remove pairs: 8/);
  ...
  assert.match(lifecycleDoc, /document listener removes: 5/);
  ...
  assert.match(source, /isolates all 42 current content-runtime\s+`document`\/`window` listener rows from the broader 288-listener inventory/);
  ...
  assert.match(source, /classifies all 29 current `requestAnimationFrame` schedules into\s+2 assigned positioning frame handles, 15 inline anonymous frame callbacks, 5\s+identifier callback frames, 5 inline `scrollIntoView` frames, 2 inline timeout\s+hop frames, 13 content-runtime frame schedules, 16 extension UI\/background\s+frame schedules, 1 `positionRaf` assignment, and 1\s+`profileDropdownPositionRaf` assignment/);
  ...
  assert.match(lifecycleDoc, /identifier callback frame schedules: 5/);
  ...
  assert.match(source, /joins all 29 current `requestAnimationFrame`\s+schedule rows and 3 current `cancelAnimationFrame` rows by direct lexical\s+handle where one exists/);
  assert.match(source, /26 frame schedule-minus-cancel delta, 27\s+frame schedules without assigned handles, 2 frame schedules with assigned\s+handles, 3 `cancelAnimationFrame` rows with direct schedule handles, 0\s+`cancelAnimationFrame` rows without direct schedule handles, 2 handled frame\s+schedule rows with cancel handles, 0 handled frame schedule rows without\s+cancel handles, 0 distinct scheduled frame handles without cancel, 13\s+content-runtime frame schedule\/cancel delta, 13 extension UI\/background frame\s+schedule\/cancel delta, 1 `positionRaf` cancel row, and 2\s+`profileDropdownPositionRaf` cancel rows/);
  ...
  assert.match(lifecycleDoc, /animation frame schedule-minus-cancel delta: 26/);
  assert.match(lifecycleDoc, /frame schedules without assigned handle: 27/);
  ...
  assert.match(lifecycleDoc, /cancelAnimationFrame rows without direct schedule handle: 0/);
  ...
  assert.match(source, /classifies all 9 current website component lifecycle\s+rows into 5 scene scheduler lifecycle rows and 4 theme sync lifecycle rows/);
  assert.match(source, /4 install-or-schedule rows, 5 explicit-teardown rows, 3\s+website component `addEventListener` rows, 3 website component\s+`removeEventListener` rows, 1 website component `setTimeout` row, 2 website\s+component `clearTimeout` rows, 2 lifecycle source files/);
  assert.match(source, /2 scene scheduler\s+install-or-schedule rows, 3 scene scheduler explicit-teardown rows, 2 theme\s+sync install-or-schedule rows, and 2 theme sync explicit-teardown rows/);
  ...
  assert.match(lifecycleDoc, /website component install-or-schedule rows: 4/);
  assert.match(lifecycleDoc, /website component explicit-teardown rows: 5/);
...
function assertRuntimeCountReconciliationAddendum(source) {
  ...
  assert.match(source, /runtime test index moved to\s+`4667` source top-level declarations/);
  ...
  assert.match(source, /current generated runtime test declarations: 4667/);
  ...
  assert.match(metricGate, /current generated runtime test declarations: 4667/);
  ...
  assert.match(runtimeTestIndex, /source top-level test declarations counted: 4667/);
  ...
  assert.equal(sourceStats.declarations, 4667);
  ...
  assert.equal(driftStats.scannedFiles, 1072);
...
function assertSourceOwnerMapDraftReadinessClosure(source) {
  ...
  assert.match(draft, /method semantic proof gap lexical callables covered: 5469/);
...
function assertMetricSampleDraftClosure(source) {
  ...
  assert.match(source, /12 closure\s+rows link all 12 sample rows, 12 inline metric sample JSON sections, 12\s+manifest contract rows, 10 artifact path boundary rows, 12 foundation packet\s+rows, 12 metric schema rows, 12 metric source-owner rows, 5 collector\s+readiness families, 63 method semantic proof gap files, and 5,469 lexical\s+callables/);
  ...
  assert.match(sample, /method semantic proof gap files covered: 63/);
  assert.match(sample, /method semantic proof gap lexical callables covered: 5469/);
  assert.match(sample, /files with complete per-callable semantic proof: 0/);
  assert.match(sample, /lexical callables requiring semantic proof before behavior changes: 5469/);
  ...
  assert.match(sample, /method semantic proof gap files linked by sample closure: 63/);
  assert.match(sample, /lexical callables linked by sample closure: 5469/);
...
function assertFixtureProvenanceDraftClosure(source) {
  ...
  assert.match(fixture, /method semantic proof gap files covered: 63/);
  assert.match(fixture, /method semantic proof gap lexical callables covered: 5469/);
  assert.match(fixture, /files with complete per-callable semantic proof: 0/);
  assert.match(fixture, /lexical callables requiring semantic proof before behavior changes: 5469/);
  ...
  assert.match(fixture, /method semantic proof gap files linked by fixture closure: 63/);
  assert.match(fixture, /lexical callables linked by fixture closure: 5469/);
...
function assertNoWorkPreservationDraftClosure(source) {
  ...
  assert.match(source, /12 closure rows link all 12 no-work preservation rows, 12\s+inline no-work preservation JSON sections, 12 fixture provenance contract\s+rows, 12 source owner map contract rows, 12 metric sample contract rows, 12\s+manifest contract rows, 10 artifact path boundary rows, 12 foundation packet\s+rows, 12 metric schema rows, 12 metric source-owner rows, 5 collector\s+readiness families, 63 method semantic proof gap files, and 5,469 lexical\s+callables/);
  ...
  assert.match(noWork, /method semantic proof gap files covered: 63/);
  assert.match(noWork, /method semantic proof gap lexical callables covered: 5469/);
  assert.match(noWork, /files with complete per-callable semantic proof: 0/);
  assert.match(noWork, /lexical callables requiring semantic proof before behavior changes: 5469/);
  ...
  assert.match(noWork, /method semantic proof gap files linked by no-work closure: 63/);
  assert.match(noWork, /lexical callables linked by no-work closure: 5469/);
...
function assertSideEffectBudgetDraftClosure(source) {
  ...
  assert.match(sideEffect, /method semantic proof gap files covered: 63/);
  assert.match(sideEffect, /method semantic proof gap lexical callables covered: 5469/);
  assert.match(sideEffect, /files with complete per-callable semantic proof: 0/);
  assert.match(sideEffect, /lexical callables requiring semantic proof before behavior changes: 5469/);
  ...
  assert.match(sideEffect, /method semantic proof gap files linked by side-effect closure: 63/);
  assert.match(sideEffect, /lexical callables linked by side-effect closure: 5469/);
...
function assertDiagnosticPrivacyDraftClosure(source) {
  ...
  assert.match(source, /12 closure rows link all 12 diagnostic privacy rows, 12\s+inline diagnostic privacy JSON sections, 12 side-effect budget contract rows,\s+12 no-work preservation contract rows, 12 fixture provenance contract rows, 12\s+source owner map contract rows, 12 metric sample contract rows, 12 manifest\s+contract rows, 10 artifact path boundary rows, 12 foundation packet rows, 12\s+metric schema rows, 12 metric source-owner rows, 5 collector readiness\s+families, 21 diagnostic logging policy source files, 418 active console\s+callsites, 63 method semantic proof gap files, and 5,469 lexical callables/);
  ...
  assert.match(diagnostic, /method semantic proof gap files covered: 63/);
  assert.match(diagnostic, /method semantic proof gap lexical callables covered: 5469/);
  assert.match(diagnostic, /files with complete per-callable semantic proof: 0/);
  assert.match(diagnostic, /lexical callables requiring semantic proof before behavior changes: 5469/);
  ...
  assert.match(diagnostic, /method semantic proof gap files linked by diagnostic closure: 63/);
  assert.match(diagnostic, /lexical callables linked by diagnostic closure: 5469/);
...
function assertParityRolloutDraftClosure(source) {
  ...
  assert.match(parity, /method semantic proof gap files covered: 63/);
  assert.match(parity, /method semantic proof gap lexical callables covered: 5469/);
  assert.match(parity, /files with complete per-callable semantic proof: 0/);
  assert.match(parity, /lexical callables requiring semantic proof before behavior changes: 5469/);
  ...
  assert.match(parity, /method semantic proof gap files linked by parity closure: 63/);
  assert.match(parity, /lexical callables linked by parity closure: 5469/);
...
function assertVerificationOutputDraftClosure(source) {
  ...
  assert.match(source, /12 closure rows link all 12 verification output rows, 12\s+inline verification output JSON sections, 12 parity rollout contract rows, 12\s+diagnostic privacy contract rows, 12 side-effect budget contract rows, 12\s+no-work preservation contract rows, 12 fixture provenance contract rows, 12\s+source owner map contract rows, 12 metric sample contract rows, 12 manifest\s+contract rows, 10 artifact path boundary rows, 12 foundation packet rows, 12\s+metric schema rows, 12 metric source-owner rows, 5 collector readiness\s+families, 63 method semantic proof gap files, 5,469 lexical callables, 4,457\s+expected runtime audit tests, 4,457 expected runtime audit passes, and 0\s+expected runtime audit failures/);
  ...
  assert.match(verification, /method semantic proof gap files covered: 63/);
  assert.match(verification, /method semantic proof gap lexical callables covered: 5469/);
  assert.match(verification, /files with complete per-callable semantic proof: 0/);
  assert.match(verification, /lexical callables requiring semantic proof before behavior changes: 5469/);
  ...
  assert.match(verification, /method semantic proof gap files linked by verification closure: 63/);
  assert.match(verification, /lexical callables linked by verification closure: 5469/);
...
test('audit_completion_gap_register_lists_required_evidence_classes', () => {
  ...
  assert.match(lifecycleDoc, /Per-file primitive footprint rows: 63/);
  for (const text of [runtimeResults]) {
    ...
    assert.match(text, /527/);
    assert.match(text, /527 exact backticked test-path/);
    assert.match(text, /0 top-level runtime test files without exact backticked entries/);
    assert.match(text, /generated per-test provenance rows: 527/);
    assert.match(text, /source top-level test declarations counted by generated index: 4667/);
  ...
  assert.match(runtimeTestIndex, /top-level runtime test files: 527/);
  assert.match(runtimeTestIndex, /indexed runtime test rows: 527/);
  assert.match(runtimeTestIndex, /source top-level test declarations counted: 4667/);
  ...
  assert.match(runtimeTestIndex, /fresh full runtime exit status for 4667 declaration set: 0/);
  assert.match(runtimeTestIndex, /current full runtime proof for generated 4667 declaration set: GO/);
  assert.match(runtimeTestIndex, /runtime results exact backticked test-path rows: 527/);
...
test('audit completion gap register records runtime lifecycle convergence without closing the goal', () => {
  ...
  assert.match(source, /pins 10 runtime lifecycle\s+convergence rows, 0 implementation-ready runtime lifecycle convergence rows,\s+510 tracked lifecycle primitive instances, 460 install-or-schedule rows,\s+50 explicit teardown rows, 16 hot YouTube SPA lifecycle owner rows, 33\s+YouTube SPA immediate\/short hot timer rows/);
