...
test('cross-feature readiness docs carry the method proof gap blocker', () => {
  for (const docPath of CROSS_FEATURE_READINESS_DOCS) {
    ...
    assert.match(doc, /method semantic proof gap lexical callables covered: 5701/, `${docPath} missing callable count`);
    assert.match(doc, /files with complete per-callable semantic proof: 0/, `${docPath} missing complete proof count`);
    assert.match(doc, /lexical callables requiring semantic proof before behavior changes: 5701/, `${docPath} missing required proof count`);
