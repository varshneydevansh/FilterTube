...
const docPath = 'docs/audit/FILTERTUBE_JSON_FIRST_ROUTE_SURFACE_METRIC_SIDE_EFFECT_BUDGET_CONTRACT_CURRENT_BEHAVIOR_2026-05-24.md';
const runtimeTestPath = 'tests/runtime/json-first-route-surface-metric-side-effect-budget-contract-current-behavior.test.mjs';
const sideEffectPath = 'docs/audit/artifacts/json-first/route-surface-metric-artifact/side-effect-budget.json';
...
const sourceDocs = {
  ...
  sourceSideEffectContract: 'docs/audit/FILTERTUBE_FIRST_OPTIMIZATION_SIDE_EFFECT_BUDGET_CONTRACT_CURRENT_BEHAVIOR_2026-05-24.md',
...
const expectedRows = [
  'FT-JSON-METRIC-SIDE-EFFECT-00-budget-identity',
  'FT-JSON-METRIC-SIDE-EFFECT-01-route-surface-mode',
  'FT-JSON-METRIC-SIDE-EFFECT-02-obligation-candidate-binding',
  'FT-JSON-METRIC-SIDE-EFFECT-03-source-owner-locus',
  'FT-JSON-METRIC-SIDE-EFFECT-04-settings-artifact-budget',
  'FT-JSON-METRIC-SIDE-EFFECT-05-transport-budget',
  'FT-JSON-METRIC-SIDE-EFFECT-06-engine-json-budget',
  'FT-JSON-METRIC-SIDE-EFFECT-07-dom-lifecycle-budget',
  'FT-JSON-METRIC-SIDE-EFFECT-08-network-storage-budget',
  'FT-JSON-METRIC-SIDE-EFFECT-09-visual-whitelist-diagnostic',
  'FT-JSON-METRIC-SIDE-EFFECT-10-no-work-parity-rollout',
  'FT-JSON-METRIC-SIDE-EFFECT-11-verification-authority'
];
...
const requiredFields = [
  ...
  'sourceFiles',
  'sourceHashStatus',
  'sourceSideEffectOwnershipStatus',
  'settingsReadBudget',
  'settingsWriteBudget',
  ...
  'passThroughBudget',
  'transportCounterBudget',
  'processDataBudget',
  'candidateExtractionBudget',
  'ruleCheckBudget',
  'decisionReportBudget',
  ...
  'mapWriteBudget',
  'messageBudget',
  'networkStorageSideEffectBudget',
  'hideMutationBudget',
...
const sideEffectBudgetAddendumPattern = /JSON-[Ff]irst [Rr]oute\/[Ss]urface [Mm]etric [Ss]ide-[Ee]ffect [Bb]udget [Cc]ontract [Aa]ddendum/;
...
test('JSON-first route/surface metric side-effect budget contract is audit-only and source-backed', () => {
  const doc = read(docPath);

  assert.match(doc, /Status: audit-only current-behavior JSON-first route\/surface metric side-effect\s+budget contract/);
  assert.match(doc, /Runtime behavior is unchanged/);
  assert.match(doc, /This is not an implementation\s+patch, optimization patch/);
  assert.match(doc, /Reserved route\/surface metric side-effect budget path: docs\/audit\/artifacts\/json-first\/route-surface-metric-artifact\/side-effect-budget\.json/);
  ...
  assert.match(doc, /Runtime route\/surface metric side-effect budget approval exists: no/);
...
test('JSON-first route/surface metric side-effect budget rows counts and fields stay pinned', () => {
  const doc = read(docPath);
  const rows = [...doc.matchAll(/^\| `(FT-JSON-METRIC-SIDE-EFFECT-[^`]+)` \|/gm)].map((row) => row[1]);

  assert.deepEqual(rows, expectedRows);
  assert.equal(rows.length, 12);
  assert.match(doc, /JSON-first route\/surface metric side-effect budget contract rows: 12/);
  assert.match(doc, /reserved route\/surface metric side-effect budget paths covered: 1/);
  assert.match(doc, /source side-effect budget contract rows covered: 12/);
  ...
  assert.match(doc, /runtime route\/surface metric side-effect budget approvals: 0/);
  assert.match(doc, /runtime route\/surface metric artifact approvals: 0/);
  assert.match(doc, /runtime metric collector approvals: 0/);
  assert.match(doc, /runtime collector side-effect approvals: 0/);
  assert.match(doc, /implementation-ready JSON-first route\/surface metric side-effect budget contract rows: 0/);
  ...
  for (const field of requiredFields) {
    assert.ok(doc.includes(field), `missing metric side-effect budget field ${field}`);
  }
});

test('JSON-first route/surface metric side-effect budget path is reserved but not committed yet', () => {
  const doc = read(docPath);

  assert.ok(doc.includes(sideEffectPath));
  assert.ok(doc.includes(foundationSideEffectPath));
  assert.equal(fs.existsSync(path.join(repoRoot, sideEffectPath)), false, `${sideEffectPath} should not exist yet`);
  assert.equal(fs.existsSync(path.join(repoRoot, foundationSideEffectPath)), false, `${foundationSideEffectPath} should not exist yet`);
});

test('JSON-first route/surface metric side-effect budget contract is backed by current NO-GO gates', () => {
  ...
  const sourceSideEffectContract = read(sourceDocs.sourceSideEffectContract);
  ...
  assert.match(sourceSideEffectContract, /Committed side-effect budget files: 0/);
  assert.match(sourceSideEffectContract, /Implementation-ready side-effect budget contract rows: 0/);
...
test('JSON-first route/surface metric side-effect budget decisions and symbols stay absent from product source', () => {
  const doc = read(docPath);
...
test('JSON-first route/surface metric side-effect budget contract is linked from ledgers and adjacent gates', () => {
  const linkedDocs = {
    ...
    sourceSideEffectContract: sourceDocs.sourceSideEffectContract,
  ...
  for (const [label, file] of Object.entries(linkedDocs)) {
    ...
    assert.match(artifact, sideEffectBudgetAddendumPattern, `${label} missing addendum heading`);
