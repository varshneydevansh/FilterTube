...
function assertMay30FreshnessAndLiveProfileBlocker(doc) {
  ...
  assert.match(doc, /current generated runtime-test declaration count: 4731/);
  assert.match(doc, /current broad runtime proof for generated 4731 declaration set: NO-GO/);
  assert.match(doc, /current broad runtime audit snapshot: 4731 tests, 4580 pass, 151 fail/);
  ...
  assert.match(doc, /current source\/runtime proof\s+-> latest recorded 4663\/4663 runtime suite proof\s+-> current 4731 generated declaration set\s+-> broad runtime backlog boundary\s+-> affected whitelist\/cache callable packet\s+-> live installed-profile evidence blocker/s);
  assert.match(doc, /flowchart TD\s+A\["Latest recorded 4663 runtime proof"\] --> B\["Current 4731 generated declaration set"\]/);
