...
test('active goal completion audit records native runtime sync method semantics without declaring completion', () => {
  ...
  assert.match(source, /current app manifest has 32 entries all sourced from `\/Users\/devanshvarshney\/FilterTube`/);
  assert.match(source, /0 `destinationKind` fields/);
  assert.match(source, /includes `js\/layout\.js`, `js\/vendor\/nanah\.bundle\.js`, `js\/vendor\/qrcode\.bundle\.js`, the managed app policy contract artifact, `js\/nanah_managed_live_policy\.js`, and `js\/nanah_managed_open_sync\.js`/);
