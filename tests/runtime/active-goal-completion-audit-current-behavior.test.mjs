...
test('active goal completion audit records no implementation-ready claim', () => {
  ...
  assert.match(source, /5,469 lexical callables are not semantic method proof/);
  assert.match(source, /63 tracked JS\/JSX\/MJS files/);
  assert.match(source, /0 files with complete\s+per-callable semantic proof/);
  assert.match(source, /5,469 lexical callables still requiring\s+semantic proof before behavior changes/);
...
test('active goal completion audit records native runtime sync manifest freshness without declaring completion', () => {
  ...
  assert.match(source, /app repo HEAD `b33e98d5b0c52cb728fb3720d34a01ba987ef649`/);
  assert.match(source, /44 dirty app paths/);
...
test('active goal completion audit records background addFilteredChannel list-target without declaring completion', () => {
  ...
  assert.match(source, /6 source\/effect blocks/);
  assert.match(source, /32 receiver lines/);
  assert.match(source, /1186 receiver bytes/);
  ...
  assert.match(source, /selected zero sender\/list-forwarding token counts/);
  assert.match(source, /11 runtime background addFilteredChannel list-target fixtures/);
  assert.match(source, /does not forward `message\.listType`/);
  assert.match(source, /declares `listType = 'blocklist'`/);
  assert.match(source, /caller-provided whitelist list type is dropped by this receiver/);
  ...
  assert.match(source, /schedule `channel_added` or `kids_channel_added` backups/);
  assert.match(source, /background addFilteredChannel still needs list-target contracts/);
  assert.match(source, /list-type forwarding policies/);
...
test('active goal completion audit records addFilteredChannel Filter All comments default without declaring completion', () => {
  ...
  assert.match(source, /32 background receiver lines/);
  assert.match(source, /1186 receiver bytes/);
  ...
  assert.match(source, /secondary background receiver drops caller-provided `filterAllComments`, forwards seven helper arguments/);
...
test('active goal completion audit records latest YTM stale menu and TMP collaborator identity gates without declaring completion', () => {
  ...
  assert.match(source, /exited 0 for the\s+current 527 runtime test files and 4667 source top-level test declarations/);
  assert.match(source, /current full runtime proof for generated 4667 declaration set:\s+`GO`/);
  ...
  assert.match(source, /legacy metric contract expected tests:\s+4457, current generated runtime test declarations: 4667, latest historical\s+4663 full runtime proof result: 4663\/4663 pass, 0 fail, 83\.213s, current full\s+runtime proof for generated 4667 declaration set after audit-drift repair:\s+GO/);
  ...
  assert.match(source, /Old `4457`\s+rows remain dated historical contract evidence only; the fresh 4667 runtime\s+pass proves executable current-behavior assertion freshness, not semantic\s+coverage, JSON-first promotion, whitelist\/cache optimization/);
  ...
  assert.match(source, /current root FilterTube audit docs: 0, current docs\/audit FilterTube audit\s+docs: 542, current root markdown docs: 36/);
  ...
  assert.match(source, /63 tracked JS\/JSX\/MJS\s+files, 5,469 lexical callables requiring semantic proof, 0 complete\s+per-callable proof files/);
  ...
  assert.match(source, /63 tracked JS\/JSX\/MJS files, 5,469 lexical callables, 0\s+complete per-callable proof files/);
  ...
  assert.match(source, /latest recorded 4663\/4663\s+runtime proof, the current 4667 generated declaration count, the later full\s+runtime freshness closure, and the May 30 whitelist\/cache live-profile blocker\s+map/);
  assert.match(source, /latest historical\s+4663 full runtime proof: 4663\/4663 pass, 0 fail, 83\.213s/);
  assert.match(source, /current generated\s+runtime-test declaration count: 4667/);
  assert.match(source, /current full runtime proof for generated\s+4667 declaration set after audit-drift repair: `GO`/);
