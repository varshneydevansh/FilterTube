...
test('tracked_file_obligation_index_links_native_runtime_sync_manifest_freshness_without_closing_rows', () => {
  ...
  assert.match(doc, /app sync script at 2284 lines and 109397 bytes/);
  assert.match(doc, /app runtime sync manifest at 212 lines and 8900 bytes/);
  ...
  assert.match(doc, /30 manifest entries/);
  assert.match(doc, /0 `destinationKind` fields/);
  assert.match(doc, /24 direct manifest copy hash matches/);
  assert.match(doc, /6 direct manifest copy hash diffs/);
  assert.match(doc, /36 broad source-mirror hash matches/);
  assert.match(doc, /2 source-mirror missing files/);
