...
test('JSON-first list-mode matrix audit is audit-only and source pinned', () => {
  ...
  for (const listModeDocPath of listModeFamilyDocs) {
    ...
    assert.match(listModeDoc, /method semantic proof gap lexical callables covered: 5469/, `${listModeDocPath} missing callable count`);
    assert.match(listModeDoc, /files with complete per-callable semantic proof: 0/, `${listModeDocPath} missing complete proof count`);
    assert.match(listModeDoc, /lexical callables requiring semantic proof before behavior changes: 5469/, `${listModeDocPath} missing required proof count`);
