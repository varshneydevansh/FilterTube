...
## 2026-08-23 post-v3.3.5 architecture checkpoint

The current unreleased architecture has four cooperating policy planes:

1. **Admission and policy** — direct Watch/Shorts/playlist/embed admission,
   per-profile daily budgets, and persisted Self-Control Sessions are enforced
   by the background/runtime boundary before page presentation.
2. **Rule collections** — Blocked and Allowed collections are independent
   storage projections; the top-level Blocklist/Whitelist policy chooses the
   default decision but does not move or erase the other collection. Complete
   BlockTube imports are validated and applied transactionally.
3. **Metadata and playback** — category hydration uses official Player
   microformat data through a bounded same-origin scheduler. Advert Void
   sanitizes the Player ad plan and quarantines escaped media without adding a
   request fan-out. Spoken language and original audio remain experimental
   Main-only consumers of the shared cache.
4. **Managed delivery** — Nanah presence, target-device/profile bindings,
   mailbox/Pickup envelopes, and native-runtime receipts are distinct from
   trust authority. A nearby device or provider cannot grant access.

The extension remains the source of truth for these contracts. Native app
resources are generated by the runtime-sync workflow and require their own
installed-device proof before parity is claimed. The complete chronology is
indexed in the [post-v3.3.5 changelog](../CHANGELOG.md).
