...
## YouTube Mobile Implementation (v3.2.8)

FilterTube v3.2.8 adds robust support for the YouTube Mobile web interface (`m.youtube.com`), which uses a distinct set of `ytm-*` renderers and a "Bottom Sheet" menu pattern.

### Mobile Menu DOM Structures (Critical for Injection)

The injection logic must differentiate between Legacy menus (Videos, Mixes) and Modern menus (Shorts/List View).
...
**Container**: `.bottom-sheet-media-menu-item` inside `bottom-sheet-container`
...
1.  **Observer**: Watches `body` for the addition of `bottom-sheet-container` or attribute changes (`open`, `class`) on existing containers.
2.  **Discrimination**:
    *   If `.bottom-sheet-media-menu-item` exists -> Use **Legacy** injection (clone `ytm-menu-service-item-renderer`).
    *   If `yt-list-view-model` exists -> Use **Modern** injection (clone `yt-list-item-view-model`).
3.  **Context Resolution**: Traces back to the `lastClickedMenuButton` to identify the source card (Video, Short, Playlist) and extract channel identity.

### High-Impact Gaps & Workarounds

*   **Mobile Playlist Panels**: `ytm-playlist-panel-video-renderer` often lacks direct channel links. We rely on the `videoId` to fetch metadata from the Main World (`ytInitialData`) or fallback to network if needed.
*   **Post-Block Enrichment**: Mobile cards are sometimes sparse. The background enrichment queue retries incomplete entries (missing handle/logo) to ensure the UI eventually shows full metadata.
*   **Optimistic Hiding**: Pending blocks apply an optimistic hide state to the container immediately to prevent "flash of unblocked content" while the async identity resolution completes.
...
### Mobile Playlist Blocking & Auto-Skip (v3.2.8)

FilterTube v3.2.8 introduces intelligent playback control for mobile playlists to prevent blocked content from playing.

#### Auto-Skip Logic
When a user plays a playlist (Mix, Radio, or standard playlist) on mobile:
1.  **Detection**: The `ytm-playlist-panel-video-renderer` elements are monitored.
2.  **State Check**: If the *currently playing* video belongs to a blocked channel (resolved via `videoChannelMap` or proactive check):
3.  **Action**:
    *   The player is immediately paused (`video.pause()`) to prevent audio leakage.
    *   The "Next" button (`.ytp-next-button`) is programmatically clicked.
    *   The skip is logged as a "Radio Skip" event.

```javascript
// In dom_fallback.js - Auto-skip logic
if (isBlockedCurrent) {
    const nextBtn = document.querySelector('.ytp-next-button:not([disabled])');
    if (nextBtn) {
        video.pause(); // Stop audio immediately
        nextBtn.click(); // Skip to next
        return; 
    }
}
```
...
When navigating through a playlist with blocked items:
*   **Forward/Backward**: The "Next" / "Previous" buttons logic in the playlist panel (`ytm-playlist-panel-video-renderer`) automatically skips hidden items.
*   `findNextVisiblePlaylistLink` scans adjacent items in the DOM to find the nearest non-blocked video to play.


## Identity Resolution Hardening (v3.2.8)

FilterTube v3.2.8 introduces several critical safety mechanisms to ensure channel identity remains deterministic and free from "stale alias" poisoning.

### 1. Round-Trip Handle Validation
To prevent false positives caused by recycled handles or stale `channelMap` entries, FilterTube now performs a **Round-Trip Validation** (`isRoundTripHandleId`).
*   **Logic**: A Handle ↔ UC ID mapping is only trusted if `channelMap[handle] === ucId` AND `channelMap[ucId] === handle`.
*   **Impact**: This prevents the extension from hiding content based on outdated aliases that no longer point to the canonical channel.

### 2. Playlist Creator Flow
Playlist collection cards (e.g., Mixes, Radio, Artist playlists) often lack direct creator links in the mobile DOM.
*   **Mechanism**: The background script implements `fetchPlaylistCreator`, which fetches the `/playlist?list=...` HTML directly.
*   **Identity Extraction**: It decodes escaped JSON blocks (`\xNN` / `\uNNNN`) and scans `playlistHeaderRenderer` to find the canonical owner, rather than relying on the first video in the list.

### 3. Card Poisoning Protection
YouTube's SPA often recycles DOM nodes. A playlist card might inherit a `videoId` from a previously rendered video card's thumbnail.
*   **Mechanism**: `dom_extractors.js` implements `isPlaylistCollectionCardElement`. If a card is identified as a playlist collection, it aggressively clears any `videoId` or `channelId` stamps derived from thumbnails.
*   **Impact**: Ensures that blocking a playlist blocks the *creator*, not the uploader of the first video shown in the thumbnail.
...
### Enhanced Visual Feedback (v3.2.8)

The menu system now provides distinct visual cues for the state of the channel being interacted with:

*   **Blocked State (Green)**: When a channel is successfully blocked, the menu item background turns subtle green (`rgba(16, 185, 129, 0.12)`), and the text turns green.
*   **Collab Selection (Red)**: When selecting a specific collaborator from a multi-channel list, the item highlights in subtle red (`rgba(248, 113, 113, 0.18)`) to indicate targeting.

```css
/* In menu.js */
.filtertube-block-channel-item.filtertube-blocked .filtertube-menu-item {
    background: rgba(16, 185, 129, 0.12) !important; /* Green tint */
}

.filtertube-block-channel-item.filtertube-collab-selected:not(.filtertube-blocked) .filtertube-menu-item {
    background: rgba(248, 113, 113, 0.18) !important; /* Red tint */
}
```
