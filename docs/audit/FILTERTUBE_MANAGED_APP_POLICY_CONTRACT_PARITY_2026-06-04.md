...
```json
{
  ...
  "runtimeHelperSync": [
    ...
    },
    {
      ...
      "manifestSyncMode": "copy",
      "boundary": "managed pull-on-open helper source parity; Internet Pickup and Home Pickup providers remain transport inputs, not policy authority"
    },
    {
    ...
    },
    {
      ...
      "manifestSyncMode": "copy",
      "boundary": "Home Pickup helper source parity; same-network reachability and discovery are transport only, local trusted-link and signature validation remain authority"
    }
  ],
  "uiHelperMirror": [
    {
  ...
  "managedChannelLists": {
    ...
    "nativeParityRequirements": [
      ...
      "preserve_managed_list_source_format_metadata",
    ...
    "nativeUiParity": {
      ...
      "copyContract": "Use parent-facing terms such as Lists, Main YouTube, YouTube Kids, Send Update, Internet Pickup, and Home Pickup. Advanced transport terms may appear only in details."
    }
  },
  "managedDelivery": {
    "transports": [
...
Internet Pickup/Home Pickup endpoint shapes locally, but it is not a hosted service,
...
`js/nanah_managed_local_network_client.js` so apps can mirror the Home Pickup
...
contract-status byte mirror. The managed timeout overlay/copy/status runtime
updates were then synced as `/Users/devanshvarshney/FilterTubeApp` commits
...
Nanah signed-send, pull-on-open, Internet Pickup, and Home Pickup helper sources
