...
| `content_script_rejects_add_filtered_channel_without_ui_owner` | Secondary `addFilteredChannel` uses `message.type`, lacks `isTrustedUiSender(sender)`, and does not forward `message.listType` to the shared helper. | `tests/runtime/p0-message-mutation-current-behavior.test.mjs` | Content-script rule adds should require allowed YouTube action proof, route/surface, row/menu target, and explicit list target. |
