...
repo-wide lexical callables: 6113
...
| --- | --- | --- |
| `method_convergence_repo_census` | 72 tracked JS/JSX/MJS files and 6,113 lexical callables are visible to the audit. | Visibility can be mistaken for behavior proof. |
| `method_convergence_zero_complete_files` | 0 files have complete per-callable semantic proof; 6,113 lexical callables still require proof before behavior changes. | A behavior patch can touch an unproved callable path while tests only cover a selected slice. |
...
```mermaid
flowchart TD
  A["72 tracked JS/JSX/MJS files"] --> B["6,113 lexical callables"]
...
repo-wide lexical callables still requiring semantic proof: 6113
files with complete per-callable semantic proof: 0
