...
repo-wide lexical callables: 6306
...
| --- | --- | --- |
| `method_convergence_repo_census` | 73 tracked JS/JSX/MJS files and 6,306 lexical callables are visible to the audit. | Visibility can be mistaken for behavior proof. |
| `method_convergence_zero_complete_files` | 0 files have complete per-callable semantic proof; 6,306 lexical callables still require proof before behavior changes. | A behavior patch can touch an unproved callable path while tests only cover a selected slice. |
...
73 tracked JS/JSX/MJS files
        |
        +--> 6,306 lexical callables
        |       +--> 3,177 hot page/background runtime
        |       +--> 3,129 UI, helper, generated, vendor, build, audit-lane, website
        |
...
```mermaid
flowchart TD
  A["73 tracked JS/JSX/MJS files"] --> B["6,306 lexical callables"]
  B --> C["3,177 hot page/background runtime callables"]
  B --> D["3,129 UI, helper, generated, vendor, build, audit-lane, website callables"]
...
| UI/settings runtime | 14 | 2094 | `semantic proof incomplete` |
...
| `js/nanah_sync_adapter.js` | UI/settings runtime | 168 | `semantic proof incomplete` | Nanah envelope, canonical payload-hash verification, trust, replay, profile target, signing-key authority, mailbox intake, local-network candidate validation, and sync parity proof remain incomplete. |
...
repo-wide lexical callables still requiring semantic proof: 6306
files with complete per-callable semantic proof: 0
