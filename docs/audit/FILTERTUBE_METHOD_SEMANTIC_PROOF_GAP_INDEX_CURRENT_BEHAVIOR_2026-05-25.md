...
method semantic proof gap lexical callables covered: 6231
...
| --- | --- | --- |
| `method_convergence_repo_census` | 73 tracked JS/JSX/MJS files and 6,231 lexical callables are visible to the audit. | Visibility can be mistaken for behavior proof. |
| `method_convergence_zero_complete_files` | 0 files have complete per-callable semantic proof; 6,231 lexical callables still require proof before behavior changes. | A behavior patch can touch an unproved callable path while tests only cover a selected slice. |
...
```mermaid
flowchart TD
  A["73 tracked JS/JSX/MJS files"] --> B["6,231 lexical callables"]
  B --> C["3,173 hot page/background runtime callables"]
  B --> D["3,058 UI, helper, generated, vendor, build, audit-lane, website callables"]
...
| UI/settings runtime | 14 | 2023 | `semantic proof incomplete` |
...
| `js/tab-view.js` | UI/settings runtime | 1078 | `semantic proof incomplete` | Dashboard profile/list-mode/Nanah/import/export/release-note action proof remains incomplete. |
...
repo-wide lexical callables still requiring semantic proof: 6231
files with complete per-callable semantic proof: 0
