...
| --- | --- | --- |
| `method_convergence_repo_census` | 69 tracked JS/JSX/MJS files and 5,789 lexical callables are visible to the audit. | Visibility can be mistaken for behavior proof. |
| `method_convergence_zero_complete_files` | 0 files have complete per-callable semantic proof; 5,789 lexical callables still require proof before behavior changes. | A behavior patch can touch an unproved callable path while tests only cover a selected slice. |
...
```mermaid
flowchart TD
  A["69 tracked JS/JSX/MJS files"] --> B["5,789 lexical callables"]
...
repo-wide lexical callables still requiring semantic proof: 5789
files with complete per-callable semantic proof: 0
