...
| `if` | no | `if (filterTubeMenuStylesInjected) {` inside `ensureFilterTubeMenuStyles()` | Control-flow artifact from the broad method-shorthand branch, not a behavior-changing callable. It remains part of the current 5,681 lexical count until the repo-wide proof layer has a callable-kind classifier. |
