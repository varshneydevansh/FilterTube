...
exact source-of-truth wording occurrences: 89
runtime source occurrences: 2
test assertion occurrences: 17
documentation/audit occurrences: 70
```
...
| `docs/THREE_DOT_MENU_IMPROVEMENTS.md:50` | `misleading-identity-effect-risk` | Collaborator roster precedence is plausible for that renderer shape, but still needs route/mode/effect proof before menu or hide behavior changes. |
| `docs/FUNCTIONALITY.md:197` | `narrow-local-owner` | Channel-derived keyword storage should be derived from channel `filterAll` state, but mutation authority remains split today. |
| `docs/CODEMAP.md:95` | `release-sync-owner` | Release notes data ownership is narrow and acceptable. |
| `docs/CODEMAP.md:21` | `release-sync-owner` | Extension-to-native source ownership is currently split across docs/data; app sync still needs source hash/provenance proof. |
