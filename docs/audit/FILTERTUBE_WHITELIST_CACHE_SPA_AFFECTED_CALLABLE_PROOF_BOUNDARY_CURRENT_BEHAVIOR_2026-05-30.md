...
| `js/background.js` | 6658 | 299580 | `f05fe6f65f9de1218299374ac3c82dd6b6ae9e17e3d862926a20e6c2981c19c7` |
...
| `FT-WLCACHE-AFFECTED-06-background-video-channel-cache` | `js/background.js:1949` `enqueueVideoChannelMapUpdate`; `js/background.js:2075` `getCompiledSettings` | Background channel-map cache updates influence compiled settings returned to content scripts. Optimization must preserve refresh ordering and profile isolation. | Anchor-only proof; implementation readiness `NO-GO`. |
| `FT-WLCACHE-AFFECTED-07-background-video-meta-cache` | `js/background.js:1974` `enqueueVideoMetaMapUpdate`; `js/background.js:2075` `getCompiledSettings` | Background metadata cache updates influence compiled settings and can drift from content cache behavior if load order or eviction changes. | Anchor-only proof; implementation readiness `NO-GO`. |
