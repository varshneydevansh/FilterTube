...
| `js/content_bridge.js` | 13624 | 603362 | `c651b34aad0ded2668a5cde55bfd4f499fab098f2f04e9ee0f50c5ede5d47b0c` | right-rail observer, visible-card prefetch, content cache, pending refresh, message branches. |
...
| `FT-WLCACHE-AFFECTED-02-right-rail-whitelist-refresh` | `js/content_bridge.js:1210` `installRightRailWhitelistObserver`; `js/content_bridge.js:1218` `runWhitelistRefreshPass` | The watch/right-rail SPA refresh path can become a lag source if it schedules fallback work on routes or modes that do not need pending whitelist processing. | Anchor-only proof; implementation readiness `NO-GO`. |
| `FT-WLCACHE-AFFECTED-03-visible-card-identity-prefetch` | `js/content_bridge.js:1382` `prefetchIdentityForCard`; `js/content/handle_resolver.js:149` `fetchIdForHandle` | Visible-card identity prefetch can improve whitelist correctness but can also add network/cache work during SPA scroll and hydration. | Anchor-only proof; implementation readiness `NO-GO`. |
| `FT-WLCACHE-AFFECTED-04-video-channel-content-cache` | `js/content_bridge.js:1629` `persistVideoChannelMapping`; `js/content_bridge.js:5903` `FilterTube_UpdateVideoChannelMap` message branch | Channel-map persistence and DOM stamping can trigger fallback reruns. Optimizing it must preserve learned channel blocking and avoid duplicate rerun churn. | Anchor-only proof; implementation readiness `NO-GO`. |
| `FT-WLCACHE-AFFECTED-05-video-meta-content-cache` | `js/content_bridge.js:1640` `persistVideoMetaMapping`; `js/content_bridge.js:1705` `scheduleVideoMetaDomRerun`; `js/content_bridge.js:5952` `FilterTube_UpdateVideoMetaMap` message branch | Metadata map persistence feeds duration/date/category filters and can schedule DOM reruns after JSON mapping. Pruning must preserve metadata correctness and avoid stale pending cards. | Anchor-only proof; implementation readiness `NO-GO`. |
...
| `FT-WLCACHE-AFFECTED-08-storage-refresh-fanout` | `js/content/bridge_settings.js:557` `scheduleSettingsRefreshFromStorage`; `js/content/bridge_settings.js:589` `handleStorageChanges` | Storage refresh coalescing controls forced reprocess behavior after rule/profile/cache updates. It must not drop force-reprocess semantics or wake heavy work for map-only changes. | Anchor-only proof; implementation readiness `NO-GO`. |
...
| `FT-WLCACHE-SEMANTIC-GAP-01-content-bridge` | `js/content_bridge.js` / Hot page/background runtime / 1202 lexical callables | Content bridge caller graph, menu/quick action authority, lifecycle callback ownership, and identity confidence proof remain incomplete. | Gap-bound only; semantic proof `NO-GO`. |
