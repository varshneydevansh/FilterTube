...
| `js/content_bridge.js` | 13536 | 600459 | `31e7234c6a4055bffb0b800bac43cf3dd1c496cb08d1d57d391ea027941277e9` | right-rail observer, visible-card prefetch, content cache, pending refresh, message branches. |
...
| `FT-WLCACHE-AFFECTED-05-video-meta-content-cache` | `js/content_bridge.js:1649` `persistVideoMetaMapping`; `js/content_bridge.js:1714` `scheduleVideoMetaDomRerun`; `js/content_bridge.js:5900` `FilterTube_UpdateVideoMetaMap` message branch | Metadata map persistence feeds duration/date/category filters and can schedule DOM reruns after JSON mapping. Pruning must preserve metadata correctness and avoid stale pending cards. | Anchor-only proof; implementation readiness `NO-GO`. |
...
| `FT-WLCACHE-AFFECTED-09-dom-fallback-pending-recheck` | `js/content/dom_fallback.js:2035` `applyDOMFallback`; `js/content/dom_fallback.js:3947` `onlyWhitelistPending` branch | DOM fallback is the broad rendered-card pass. It must keep cheap gates before selector traversal and avoid running full fallback when only whitelist pending rows are needed. | Anchor-only proof; implementation readiness `NO-GO`. |
