...
| JSON-first filtering | `test:json` keeps seed/network, readiness, snapshot, list-mode, response-mutation, and YTM showSheet collaborator parity/enrichment tests in lane. |
...
| settings | `test:settings` keeps settings-mode, settings source/effect, claim-register authority, compiled-settings field/register proof, content-control alias mutation, refresh fanout, cross-context consumers, compiled-cache, import/export, and state-manager tests in lane. |
...
`4731` tests ran, `4610` passed, and `121` failed in the latest sampled TAP
