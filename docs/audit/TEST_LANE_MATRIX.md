...
| lane workflow file-size guard | `npm run test:release`, `npm run test:performance`, `npm run test:smoke` | Keeps workflow-owned lane files below 1000 lines. If a lane workflow file approaches the boundary, split the matrix or runner proof before adding more assertions. |
