...
| `js/content_bridge.js` | `test:menu`, `test:settings`, `test:blocking`, affected runtime lane | The bridge mutates hide/block decisions, action targets, collaborator identity, and DOM fallback state. The menu lane owns collaborator identity promotion handoff proof, including the ampersand Topic guard; add `test:whitelist`, `test:dom`, `test:json`, or `test:performance` depending on touched branch. |
