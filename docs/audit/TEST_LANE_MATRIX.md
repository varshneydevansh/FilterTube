...
| diagnostic, logging, console, no-work, cache, SPA, lag, active-work, or performance audit docs under `docs/audit/` | `test:performance`, `test:smoke` | Covers audit proof for production console gates, diagnostic logging budgets, empty-rule/no-work behavior, cache/SPA lag, and performance-risk boundaries even when only the proof doc changes. |
