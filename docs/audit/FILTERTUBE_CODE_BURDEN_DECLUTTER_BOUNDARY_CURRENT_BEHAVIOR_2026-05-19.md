...
| `js/nanah_sync_adapter.js` | 1741 | Nanah portable payload, managed-policy apply surface, WebCrypto verifier helper, managed public-key descriptor advertisement, source signing-key helpers, canonical managed-policy payload-hash verification, source-side server-safe encrypted mailbox storage item construction, local WebCrypto mailbox seal/open helpers, local decrypted managed mailbox item intake, and validation-only local-network managed-policy candidate gate. Current growth is owned by the managed-policy integrity, encrypted mailbox, local seal/open helper, and LAN candidate authority slices; follow-up should extract managed-policy helpers behind the same validation tests before broadening transport. |
