...
| `js/tab-view.js` | 13007 | Dashboard/settings UI surface. Current residual growth is owned by the managed Nanah live signed-send, explicit granular rule-source UI, managed mailbox intake, and managed parent status surface slices after extracting policy construction to `js/nanah_managed_live_policy.js`; any further growth needs UI/state owner proof and release smoke. |
