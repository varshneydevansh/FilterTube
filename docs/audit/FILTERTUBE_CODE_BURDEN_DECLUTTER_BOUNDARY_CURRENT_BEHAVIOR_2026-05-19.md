...
near-threshold product-owned JS/JSX/MJS files from 900 to 999 lines guarded: 3
...
| --- | ---: | --- |
| `js/tab-view.js` | 14984 | Dashboard/settings UI surface. Current residual growth is owned by the managed Nanah live signed-send sensitive re-auth gate, managed provider-delivery re-auth dependency wiring, active/full managed profile-policy bundle expansion UI, live accepted/rejected ack receive handling, rule-bundle send expansion UI, explicit granular rule-source UI, managed mailbox intake, managed mailbox purge-provider removal handoff, managed parent status surface, delegated managed command-center action intents, managed action-history clear-evidence, remote failed-attempt rate-limit metadata, provider-gated open-sync UI wiring, provider-gated mailbox ack-history recording, provider-gated local-network candidate discovery/status handoff, provider-gated source-side delivery ack status recording, persisted managed-admin failed-unlock rate-limit slices, trusted-link revocation cleanup, outbound/live ack history metadata plumbing, sanitized local-network candidate receive/history/apply handling, locked-child revision-bound managed update gating, local managed time-limit policy-history row construction, local managed viewing-space policy-history row construction, managed action-history display redaction, selected-profile bulk time-limit policy application, selected-profile bulk viewing-space policy application, and timezone validation before dashboard managed time-limit policy reads are armed; any further growth needs UI/state owner proof and release smoke. |
| `js/content_bridge.js` | 13636 | Giant cross-context hub. Any growth needs source-owner proof and focused runtime fixtures. |
| `js/background.js` | 6803 | Background storage/message/profile authority. Current residual growth includes managed time-limit timezone validation before policy import and compile; any further growth needs mutation and settings proof. |
...
| `js/io_manager.js` | 2111 | Import/export and backup surface. Current residual growth includes managed time-limit timezone validation before imported policy persistence; any further growth needs payload and migration proof. |
| `js/popup.js` | 1841 | Popup settings/action UI. Any growth needs settings UI proof and popup smoke. |
| `js/nanah_sync_adapter.js` | 1756 | Nanah portable payload, managed-policy apply surface, WebCrypto verifier helper, managed public-key descriptor advertisement, source signing-key helpers, canonical managed-policy payload-hash verification, source-side server-safe encrypted mailbox storage item construction, local WebCrypto mailbox seal/open helpers, local decrypted managed mailbox item intake, validation-only local-network managed-policy candidate gate, and managed time-limit timezone validation before applied policy persistence. Current growth is owned by the managed-policy integrity, encrypted mailbox, local seal/open helper, LAN candidate authority, and time-limit timezone authority slices; follow-up should extract managed-policy helpers behind the same validation tests before broadening transport. |
...
| `js/content/bridge_settings.js` | 1127 | Content-script settings relay and managed time-limit runtime. Current residual growth includes managed time-limit timezone validation before content runtime arming; any further growth needs settings, route, and no-work proof. |
...
| `js/nanah_managed_live_policy.js` | 1113 | Managed live-send, local-network delivery, mailbox upload handoff, mailbox purge handoff, source/provider ack handling, and provider-delivery re-auth helper. This file has crossed the 1000-line threshold; any further delivery growth should split provider transport helpers or add explicit owner/decomposition proof before broadening the mailbox/local-network surface. |
