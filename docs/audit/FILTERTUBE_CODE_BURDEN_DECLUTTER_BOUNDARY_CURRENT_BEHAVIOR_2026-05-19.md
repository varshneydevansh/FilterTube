...
| `js/nanah_sync_adapter.js` | 1419 | Nanah portable payload, managed-policy apply surface, WebCrypto verifier helper, managed public-key descriptor advertisement, source signing-key helpers, canonical managed-policy payload-hash verification, and local decrypted managed mailbox item intake. Current growth is owned by the managed-policy integrity slice; follow-up should extract managed-policy helpers behind the same validation tests before broadening transport. |
