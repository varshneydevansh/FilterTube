...
The tracked manifest surface has 4 files, 336 newline counts, and 9,409 bytes:

| Path | Lines | Bytes | SHA-256 | Current package role |
| --- | ---: | ---: | --- | --- |
| `manifest.json` | 87 | 2,470 | `96eb5e5c8733ecdfa9d3eb447d51a3bfc2c4743a80b1fde1f12d71bd46d1c8e4` | Default/unpacked Chromium-style manifest |
| `manifest.chrome.json` | 87 | 2,470 | `96eb5e5c8733ecdfa9d3eb447d51a3bfc2c4743a80b1fde1f12d71bd46d1c8e4` | Build input for Chrome/Edge/Brave ZIP |
| `manifest.firefox.json` | 74 | 1,994 | `5d7175c23dbce4f9e86b0db0f34b1ae61bb465a9725ff37fc7069a45d4ceac5c` | Build input for Firefox ZIP |
| `manifest.opera.json` | 88 | 2,475 | `f76d4a48b51fc5da65492347ce3f7cb31ebff057afd2185573176991e7d1d4b7` | Build input for Opera ZIP |
...
| --- | --- | ---: | --- | --- | --- |
| `manifest.json` | MV3 `service_worker: js/background.js` | 2 | first entry: `js/seed.js`, `world: MAIN`, `document_start` | second entry: 13 scripts, `world: ISOLATED`, `document_start` | `js/injector.js`, `js/filter_logic.js`, `js/seed.js`, `js/shared/identity.js`, `icons/file.svg` |
| `manifest.chrome.json` | MV3 `service_worker: js/background.js` | 2 | first entry: `js/seed.js`, `world: MAIN`, `document_start` | second entry: 13 scripts, `world: ISOLATED`, `document_start` | same as default |
| `manifest.firefox.json` | `background.scripts`: `js/shared/identity.js`, `js/background.js` | 1 | no manifest MAIN-world seed; fallback injection pushes `seed` in `js/content/bridge_injection.js` | one 13-script stack with no `world` field, `document_start` | same as default |
| `manifest.opera.json` | MV3 `service_worker: js/background.js` | 2 | first entry: `js/seed.js`, no explicit `world`, `document_start` | second entry: 13 scripts, no explicit `world`, `document_start` | runtime JS resources only; no `icons/file.svg` |
...
Current source has 55 content-script file references across all manifests and
14 unique content-script files. All referenced content-script files are tracked.
