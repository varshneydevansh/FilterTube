...
The 2026-06-21 background retention/redaction alignment slice makes the timeout
overlay's `FilterTube_ManagedTimeLimitParentRequest` writer use the same 30-day
accepted-action, 90-day protected-evidence, 500-row cap, and safe summary-key
redaction model as dashboard-side history writes. This keeps
`policy.time_limit.request_extra` rows protected even though they are written
from the background service worker after runtime budget revalidation, preserves
only safe counts/status/transport metadata, and does not grant time, unlock
YouTube, or alter the parent approval flow.
