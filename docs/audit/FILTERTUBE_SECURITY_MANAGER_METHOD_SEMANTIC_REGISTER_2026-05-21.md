...
| Background session cache | `js/background.js:716-759`, `js/background.js:3603-3615` | Extracts profile/Master verifier, caches authorized profile PINs with an expiry after `FilterTube_SessionPinAuth`, and only some actions consult `isProfileSessionAuthorized()`. | Session checks are not applied consistently to every rule/list/profile mutation action. |
