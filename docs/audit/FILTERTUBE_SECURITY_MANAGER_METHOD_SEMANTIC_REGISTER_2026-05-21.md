...
| Background session cache | `js/background.js:716-871`, `js/background.js:3603-3615` | Extracts profile/Master verifier, caches authorized profile PINs with an expiry after `FilterTube_SessionPinAuth`, keeps PINs memory-only, persists failed-attempt rate-limit state under `profile.managedPolicyState.adminFailedUnlockRateLimit`, and gates batch whitelist import plus list-mode mutation through `isProfileSessionAuthorized()`. | Session checks are not applied consistently to every remaining rule/list/profile mutation action. |
