...
| Dashboard unlock wrapper | `js/tab-view.js:9126-9174` | Calls `verifyPin()`, records `sessionMasterPin` for Default, marks a TTL-bound local unlock session, and forwards session auth to background. | Dashboard session state is local and not a shared mutation authority. |
| Background session cache | `js/background.js:716-871`, `js/background.js:3603-3615`, `js/background.js:4729-4737`, `js/background.js:5150-5172`, `js/background.js:6421-6456`, `js/background.js:7185-7207` | Extracts profile/Master verifier, caches authorized profile PINs with an expiry after `FilterTube_SessionPinAuth`, keeps PINs memory-only, persists failed-attempt rate-limit state under `profile.managedPolicyState.adminFailedUnlockRateLimit`, and gates batch whitelist import, list-mode mutation, whitelist transfer, channel add/update helpers, the legacy persistent channel add path, and Filter All toggles through `isProfileSessionAuthorized()`. | Session checks still need to be proven against every remaining settings/profile/import mutation action before the security manager can be treated as full mutation authority. |
...
| IO encrypted export/import | `js/io_manager.js:1796-1838` | Calls `encryptJson()` and `decryptJson()` around backup payloads; encrypted import delegates to `importV3()` without forwarding `targetProfileId`. | Encrypted payload validity is not profile targeting authority or Nanah trust authority. |
| Background encrypted backup | `js/background.js:901-919` | Calls `encryptJson()` with the TTL-checked cached active-profile PIN or skips with `missing_session_pin`. | Backup scheduling, skip reporting, and rotation authority are owned outside `security_manager.js`. |
| Dashboard encrypted import decrypt | `js/tab-view.js:10120-10136` | Calls `decryptJson()` after prompting for the file password/PIN. | Decryption success still needs import preview, trusted-state choice, and mutation authority. |
