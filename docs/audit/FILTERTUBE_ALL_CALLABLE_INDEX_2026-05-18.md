...
| UI/settings runtime | 12 | 1869 | First-pass UI/settings audit exists, but action-level fixtures remain incomplete. |
...
| `js/nanah_managed_live_policy.js` | UI/settings runtime | 42 | UI/settings counted |
...
| `js/tab-view.js` | UI/settings runtime | 1008 | UI/settings counted |
...
2. **`js/tab-view.js` is the largest UI surface.**
   The broad parser finds 1,008 callable forms. Any dashboard change can cross
...
3. **The hot runtime is not the whole product.**
   ...
   generated shell, vendor, build scripts, and website add another 2,902
...
- documented family totals sum to 6,072,
