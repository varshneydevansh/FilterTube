...
| UI/settings runtime | 10 | 1689 | First-pass UI/settings audit exists, but action-level fixtures remain incomplete. |
...
| `js/nanah_sync_adapter.js` | UI/settings runtime | 100 | UI/settings counted |
...
| `js/tab-view.js` | UI/settings runtime | 930 | UI/settings counted |
...
2. **`js/tab-view.js` is the largest UI surface.**
   The broad parser finds 930 callable forms. Any dashboard change can cross
...
3. **The hot runtime is not the whole product.**
   ...
   generated shell, vendor, build scripts, and website add another 2,722
...
- documented family totals sum to 5,892,
