...
| Build/package boundary | `build.js` copies the `css` directory, but not the `design` directory. `package.json` has no script whose name or command mentions design tokens, and `build.js` does not reference `design/design_tokens.json`. |
