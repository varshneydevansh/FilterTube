...
- Trusted-link removal now purges target-local accepted managed-policy revision
  ...
  mailbox protocol still has no FilterTube-hosted runtime server queue.
  Configured-provider purge can request deletion from an explicit endpoint after
  parent/account re-auth, and source-side trust removal can build a
  provider-gated source-side mailbox purge request for pending ciphertext rows.
...
| Mailbox protocol specified, runtime partially hooked | Offline later delivery now has a ciphertext-only protocol, local WebCrypto seal/open helpers, source-side server-safe storage item builder, source-side provider upload handoff, source-side provider purge handoff, replay/ack proof fixture, and a browser-side HTTPS configured-provider client for upload, pull, ack, and purge. The dashboard/profile-open hook can ask a trusted provider for already-decrypted mailbox items and return redacted ack records after local apply/reject. Parent/account mailbox provider configure/disable actions now write redacted per-protected-profile history rows with configured state and endpoint host only. Provider rejection or provider failure fails closed without applying or acknowledging returned items; no FilterTube-hosted mailbox server path exists yet. | Pairing key persistence plus signed live-delivery tests before mailbox transport work. |
...
| App policy contract parity now explicit | The extension-owned contract names the fields apps must preserve, the app sync manifest copies a dedicated contract artifact, Android now has model-level proof for managed policy state/action history preservation plus Activity startup/resume/heartbeat/pause enforcement for managed time budgets, and the sibling app repo has committed the latest managed delivery/runtime sync plus managed list UI contract/status mirror as `/Users/devanshvarshney/FilterTubeApp` commits `cb8e1516 Sync managed delivery runtime from extension`, `2994f91a Sync managed list UI contract from extension`, and `01b7e695 Mirror synced managed list contract status`. The newer managed timeout overlay, optional delivery copy, and passive time-left status mirrors were synced as `cc5d24f3 Sync managed timeout overlay runtime`, `88a7a39d Sync optional delivery setup copy`, and `f13b03c7 Sync managed time remaining runtime`. Native Android/iOS source parity now also covers reviewed rule-list imports, saved-update pickup checks, Android managed time-budget enforcement, and iOS generated runtime heartbeat/local-budget/timeout-overlay/request-more-time hooks; `/Users/devanshvarshney/FilterTubeApp` commit `b6b317bfe Document native rule list import parity` records the current rule-list boundary. The installed app parity smoke verifier/template remains the Android/iOS release evidence gate. Rich native timeout UI, settings locks, and executed installed-device app smoke remain pending. | Execute and verify installed Android/iOS Main/Kids route, time-budget, saved-update pickup, native settings-lock, and child/admin authority separation smoke through `docs/audit/artifacts/managed-app-parity-smoke/verify-managed-app-parity-smoke-artifact.mjs`. |
