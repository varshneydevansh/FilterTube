...
| Live and provider-delivery admin locks present; bulk UI locks still pending | Signed live Source -> Replica managed sends require the active parent/account profile to pass a sensitive-action unlock before envelopes are built or sent. Source-side mailbox/local-network provider delivery now uses the same sensitive unlock before batch signing, provider calls, or sent-state marking. Future richer remote-delivery and bulk-command UI writes still need equivalent fixtures before exposure. | Keep live-send and provider-delivery re-auth fixtures passing; add bulk-command admin-lock fixtures before exposing richer writes. |
