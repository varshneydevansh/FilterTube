...
| Partial protected-user action history | Accepted local managed child saves, local parent/account history access, accepted-row clearing, protected failed unlock rows, dashboard/background session expiry, sensitive managed-action re-auth, profile-persisted local/background failed-attempt rate limiting, remote managed validation/apply rows, central redacted-summary sanitizing, 30-day accepted-action retention, 90-day protected-evidence retention, and the 500-row profile cap exist. | Encrypted summary fixtures and live remote apply smoke. |
...
| Partial protected log access policy | Local child history viewer, accepted-row clear path, profile-persisted failed-attempt state, runtime retention pruning, and central summary sanitizing now preserve protected evidence while bounding local history size and stripping plaintext payload fields; encrypted summaries remain pending. | Encryption proof and installed remote-history smoke. |
