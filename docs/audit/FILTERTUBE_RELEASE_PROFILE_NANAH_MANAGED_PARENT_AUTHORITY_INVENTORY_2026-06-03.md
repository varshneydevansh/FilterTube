...
| App policy contract parity now explicit | The extension-owned contract names the fields apps must preserve, the app sync manifest copies a dedicated contract artifact, and Android now has model-level proof for managed policy state/action history preservation plus pure time-budget decisions. Activity-wide Android entry/continuation enforcement, settings locks, and iOS parity remain pending. | Android Activity/WebView adapter tests for Main/Kids route gates and time limits, native settings-lock tests, iOS adapter parity, and child/admin authority separation fixtures. |
