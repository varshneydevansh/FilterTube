...
| `list_mode_compile_owner` | `js/background.js:2424-2488`; `js/background.js:2522-2542`; `js/background.js:2678-2690` | Background compiles `profileType`, `listMode`, whitelist allow sources, and blocklist canonical-then-alias sources, with Kids-to-Main merge only when modes match. | Runtime block/allow behavior depends on compiled source selection, not only stored UI rows. |
