...
| `list_mode_compile_owner` | `js/background.js:1991-2045`; `js/background.js:2079-2099`; `js/background.js:2235-2247` | Background compiles `profileType`, `listMode`, whitelist allow sources, and blocklist canonical-then-alias sources, with Kids-to-Main merge only when modes match. | Runtime block/allow behavior depends on compiled source selection, not only stored UI rows. |
