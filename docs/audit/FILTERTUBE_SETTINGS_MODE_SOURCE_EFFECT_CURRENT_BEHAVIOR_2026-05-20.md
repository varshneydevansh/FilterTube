...
| `list_mode_compile_owner` | `js/background.js:2346-2410`; `js/background.js:2444-2464`; `js/background.js:2600-2612` | Background compiles `profileType`, `listMode`, whitelist allow sources, and blocklist canonical-then-alias sources, with Kids-to-Main merge only when modes match. | Runtime block/allow behavior depends on compiled source selection, not only stored UI rows. |
