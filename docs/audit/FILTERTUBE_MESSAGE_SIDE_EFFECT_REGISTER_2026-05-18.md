...
| `addFilteredChannel` | `ruleMutation`, `storageWrite`, `backupSchedule`, `learnedIdentity` | Secondary `message.type` listener. | `js/background.js` calls `handleAddFilteredChannel()` but does not forward `message.listType`. | Explicit sender class, profile/list target, lock state, and menu-action proof. |
