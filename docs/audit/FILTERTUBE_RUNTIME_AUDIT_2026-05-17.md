...
| Path | Proof | Risk |
| --- | --- | --- |
| Background compiled-settings authority | `js/background.js:1774-1830`, `js/background.js:3242-3265`. | Background can compile and cache canonical runtime settings, but compilation still reads broad storage and can perform migration/derived writes elsewhere in the function. It needs a pure compiler boundary. |
...
| 3-dot/native persistent add | Second listener `type:addFilteredChannel` at `js/background.js:5218-5247`; shared helper at `js/background.js:5276-6167`; callers `js/content/block_channel.js:1199`, `js/content_bridge.js:12806`, `js/state_manager.js:666`; fixtures `background currently has two separate channel-add implementations` and `addFilteredChannel message path currently does not forward listType to handleAddFilteredChannel`. | Writes many profile/list/map keys and may fetch channel identity. | `INSTRUMENT` | Better central helper, but it still needs sender/profile/list/write declarations before it becomes the only mutation path. The message bridge also currently drops `listType`, so the helper's allow/block parameter is not actually honored by that entry point. |
...
| Compile-time writes | `js/background.js:2080`, `js/background.js:2397`. | Runtime compilation can be a migration writer; tests must assert both output payload and side effects. |
...
| Shared add/filter helper | Secondary listener `addFilteredChannel` / `toggleChannelFilterAll` at `js/background.js:5209-5259`; helper at `js/background.js:5276-6145`; fixture `addFilteredChannel message path currently does not forward listType to handleAddFilteredChannel`. | Any runtime sender. | User-rule mutation, identity fetch, channelMap write, compatibility write, backup, refresh. | This is the better central path, but it currently accepts multiple intents without a declared transaction phase, and one entry point drops the explicit list destination before the helper receives it. | Decompose into `resolveIdentity`, `validateIntent`, `writeRule`, `writeCompatibilityMirror`, `refreshRuntime`, each with result fields. |
...
| --- | --- | --- | --- | --- |
| Compiler cache write | `js/background.js:1774-1781`, `js/background.js:2553-2558` | `getCompiledSettings()` returns cache unless forced, and writes compiled settings after a storage read. | Correct only if invalidation exactly matches all compiler dependencies. | `KEEP` + schema |
...
| Video-channel map cache mutation | `js/background.js:1648-1670` | `enqueueVideoChannelMapUpdate()` updates pending map writes and copies the new mapping into cached Main/Kids settings. | Can change filtering identity without a full compile. Needs source/freshness/provenance. | `KEEP` + provenance |
| Video-meta map cache mutation | `js/background.js:1673-1712` | `enqueueVideoMetaMapUpdate()` may mutate `videoMetaMapCache`, then points cached Main/Kids settings at that cache. | Category/duration/upload filters can observe metadata changes through cache mutation instead of compile. | `INSTRUMENT` |
...
| Background map cache mutation | `js/background.js:1495-1512`, `js/background.js:1648-1670`, `js/background.js:1673-1712` | Updates persistent queues and mutates cached compiled settings maps. | Passive learning mutates the runtime payload cache instead of producing an explicit map-overlay event. | `REWRITE` overlay layer |
...
| Video/channel map harvest | `js/filter_logic.js:1335-1370`, `js/content_bridge.js:1465-1487`, `js/background.js:1648-1665`. | Useful, but stores plain `videoId -> channelId` without provenance, freshness, or confidence. | `KEEP` + `INSTRUMENT` |
...
| Mobile artifact collection | `build.js:34`, `build.js:214-272`. | Optionally stages Android APK/AAB artifacts and writes SHA-256 files. | Good direction for public APK release, but source directory defaults to `release-artifacts/mobile` while earlier app release docs used `release-artifacts/android-mobile-tablet`. | `REWRITE` path contract |
| Latest mobile artifact selection | `build.js:246-278`. | Picks latest versionCode unless `--all-mobile-artifacts` is set. | Correct for releases, but needs explicit refusal if latest APK and AAB versionCodes differ. | `GATE` pairing check |
...
| Mobile artifact regex | `build.js:34`. | Accepts `FilterTube-mobile-tablet-v...-code...-(release|debug).(apk|aab)`. | Good naming guard, but package id/signing/version cannot be proven from filename. | `GATE` artifact metadata proof |
| `maybeCollectMobileArtifacts()` | `build.js:214-272`. | Reads a source directory, selects matching Android artifacts, copies them into `dist/mobile`, writes SHA-256 files. | Default directory is `release-artifacts/mobile`, while the app release workflow used `release-artifacts/android-mobile-tablet`. This can make release scripts miss the intended artifacts. | `REWRITE` path contract |
| `selectLatestMobileArtifacts()` | `build.js:275-283`. | Chooses files with max parsed versionCode. | If release AAB and direct APK are not both present for that versionCode, the release can publish an incomplete Android surface. | `GATE` APK/AAB pair check |
...
| --- | --- | --- | --- | --- |
| `getCompiledSettings()` cache hit and storage read | `js/background.js:1774-1828` | Reads a broad storage key set, returns cached payload unless `forceRefresh` is true. | Cache correctness depends on every writer invalidating exactly the same dependency set. | `KEEP` cache, `REWRITE` dependency schema |
...
| Learned video/channel map updates | `js/background.js:1594-1627`, `js/background.js:1648-1712`, `js/background.js:4397-4422` | Runtime messages enqueue/persist `channelMap`, `videoChannelMap`, and `videoMetaMap`; cache maps are mutated in-place. | Passive learning alters future identity decisions and compiled payloads without profile/route/confidence provenance. | `GATE` |
