...
| Managed filter lists | Parent can use one `Lists` action to view imported-list summaries with last-checked/hash metadata, import pasted/file/public-HTTPS channel lists, preview valid rows, apply them to selected protected profiles and Main/Kids surfaces, see compact list status, pause/resume saved lists, refresh one or all saved URL-backed lists manually, remove list-derived rows without touching manual rules, and push the resulting channel policy to verified devices. Future UI should add scheduled subscription refresh. | A list URL/file is only a rule source. It must not become authority, executable code, or an invisible global block. Parent/admin approval, source labeling, revision/hash metadata, disable/revert behavior, and protected action history are required. |
