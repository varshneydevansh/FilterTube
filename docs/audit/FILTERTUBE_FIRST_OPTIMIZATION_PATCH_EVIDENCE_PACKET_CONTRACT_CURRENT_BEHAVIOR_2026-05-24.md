...
| `FT-EVIDENCE-08-diagnostic-privacy` | The patch must account for diagnostic logging and privacy while measuring or removing logs. | Current runtime has 418 active console callsites without one diagnostic measurement policy. | Diagnostic budget, log owner, reason, route, profile, privacy class, redaction policy, debug gate, and metric replacement. |
