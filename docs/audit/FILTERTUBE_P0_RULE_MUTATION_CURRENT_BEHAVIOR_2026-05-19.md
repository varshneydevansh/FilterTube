...
| `rule_mutation_report_exists_for_background_add_filtered_channel` | Not satisfied today. | `js/background.js:4095-4380` handles `addChannelPersistent`; `js/background.js:5209-5246` handles secondary `addFilteredChannel` and omits `message.listType`, so the helper defaults to blocklist. | Quick/menu/native writes can add rules through a different trust and list-target path than popup/tab UI. |
