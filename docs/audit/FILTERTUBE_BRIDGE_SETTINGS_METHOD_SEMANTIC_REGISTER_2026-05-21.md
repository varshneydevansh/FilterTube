...
| `if` artifacts | 42 | no | The broad method-shorthand branch misclassifies `if (...) {` control-flow lines as lexical callables. These remain part of the current 5,469 lexical count until the repo-wide proof layer has a callable-kind classifier. |
