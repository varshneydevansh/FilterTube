...
hardening, and a shared managed-admin authority helper are present for
protected-profile edits in the extension dashboard.
...
The current product source already blocks child/protected-user profiles from
becoming admins through this local edit path. The runtime slices preserve that
behavior, support non-Default protected profiles below a managing account or
the master profile, and add
...
| `startManagedChildEdit(profileId, surface)` | Loads fresh profiles, rejects Default or missing targets, requires `canActiveProfileManageProfile(...)`, then requires `ensureProfileUnlocked(...)` for the active parent/account profile. Despite the legacy function name, the target is a managed protected profile, not only a literal child row. |
| `saveManagedChildSurface(surface, mutator)` | Requires an active managed protected-profile edit target, reloads fresh profiles, rejects a missing target or Default, reruns `canActiveProfileManageProfile(...)`, writes only the protected target's selected surface, records local revision/history metadata, then reloads UI state. |
...
| `master_manages_independent_protected_profile` | Default/Master profile | Non-Default account/protected profile | Allowed after master/admin unlock when required; Default is never edited through this virtual protected-profile path. |
...
| `save_rechecks_fresh_protected_target` | Parent/admin save from stale edit state | Missing target, Default profile, or no-longer-managed target | Rejected during save before any profile surface mutation. |
