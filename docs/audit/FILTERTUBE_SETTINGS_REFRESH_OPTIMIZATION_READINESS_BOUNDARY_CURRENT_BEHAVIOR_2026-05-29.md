...
| `FT-SRFO-04-identity-prefetch-fanout` | `js/content_bridge.js::refreshFilterTubeRuntimeObservers`, `schedulePrefetchScan` | Starts card prefetch observation only when identity prefetch or collaborator warmup work is needed; otherwise clears the prefetch queue. | Needs route/surface/list-mode identity and collaborator-warmup budgets before changing prefetch scheduling. |
