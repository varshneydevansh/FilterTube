...
| `stats_storage_write_is_batched_or_debounced` | Satisfied on 2026-08-27. | `saveStats()` coalesces a synchronous hide/restore burst and serializes a follow-up write when counters change in flight. | Large Home-feed hide bursts no longer issue one storage read/write per card. |
