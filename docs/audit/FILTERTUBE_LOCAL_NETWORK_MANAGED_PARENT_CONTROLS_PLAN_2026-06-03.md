...
- [x] 2026-06-26 pickup provider status proof slice: the provider ownership
  ...
  decisions. The reference provider test verifies that `GET /filtertube`
  returns a transport-only status response without obvious rule or PIN fields,
  so release proof cannot skip the address parents/admins are told to inspect.
