...
| DOM selector, hide, and lifecycle registers | DOM selector instance register, lifecycle primitive inventories, tab-view lifecycle selector boundary | `direct-hide-writer-register` has been refreshed and promoted into `test:dom`; remaining DOM/lifecycle inventories have current-source drift that belongs in smaller DOM/performance batches. |
