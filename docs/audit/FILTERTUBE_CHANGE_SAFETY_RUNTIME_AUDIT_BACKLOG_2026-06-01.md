...
| Settings and content-control registers | settings mode source/effect, settings refresh cross-context consumer rows, source-of-truth claim register | `compiled-settings-field-register` has been refreshed and promoted into `test:settings`; `content-control-active-work-matrix` has been refreshed and promoted into `test:performance`; `content-control-alias-mutation-boundary` has been refreshed and promoted into `test:settings`; `settings-mode-source-effect` and `source-of-truth-claim-register` have been refreshed and promoted into `test:settings`; remaining rows are settings refresh cross-context consumer cleanup. |
