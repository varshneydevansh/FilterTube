...
| Settings and content-control registers | compiled settings fields, content-control active-work matrix, content-control alias mutation | `compiled-settings-field-register` has been refreshed and promoted into `test:settings`; the remaining content-control registers still need refreshed source/effect rows before serving as broad proof. |
