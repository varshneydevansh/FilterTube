...
| `backgroundEnabledFromV4` | `js/background.js:367` | 10 | 359 | Background compilation resolves `enabledFromV4` from profile settings, falling back to legacy `items.enabled !== false`. |
| `backgroundBooleanPassThrough` | `js/background.js:2956` | 34 | 3,529 | Background compiled settings assign `compiledSettings.enabled = enabledFromV4` with other boolean flags. |
| `backgroundInvalidationKeys` | `js/background.js:4970` | 16 | 461 | Background storage-change cache invalidation does not include `enabled`. |
...
| `bridgeRefreshKeys` | `js/content/bridge_settings.js:1075` | 44 | 1,263 | Content bridge storage refresh includes `enabled`. |
