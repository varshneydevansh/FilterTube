...
| --- | ---: | ---: | --- |
| Build and sync scripts | 4 | 31 | Extension ZIPs, generated UI shell, vendor Nanah/QR bundles, native app runtime sync, GitHub release body/assets |
...
| Total | 26 | 82 | Public release and website truth boundary |

## Accounted Files

| File | Count | Important callable / surface |
| --- | ---: | --- |
| `build.js` | 25 | `main`, `maybeCollectMobileArtifacts`, `buildReleaseBody`, `createGitHubRelease`, `uploadReleaseAsset`, `updateReadmeBadges` |
...
| --- | --- | --- | --- |
| Extension package contents | `build.js` copies `js`, `css`, `html`, `icons`, `data`, and `assets`, then writes browser-specific `manifest.json` and zips each target. | `build.js:30`, `106`, `120`, `131`, `145`, `181` | Release ZIP content is broad; quarantined or generated files must stay classified before packaging. |
...
| README mutation during build | `build.js` always calls `updateReadmeBadges(VERSION)`, then rewrites `README.md`. | `build.js:84`, `602`, `636`, `657` | A normal package build mutates public docs, so release automation can create unrelated dirty changes or publish badge-only drift. |
| Mobile artifact staging | Android artifacts are opt-in via CLI/env/prompt, filtered by filename, copied to `dist/mobile`, and checksummed. | `build.js:34`, `55`, `214`, `237`, `255`, `262`, `265` | Good checksum baseline, but no release manifest proves signing fingerprint, Play/internal state, or website CTA readiness. |
| GitHub release creation | The script creates a non-draft, non-prerelease GitHub release before uploading assets. | `build.js:327`, `340`, `482`, `487`, `507` | If asset upload fails after release creation, users can see a public release missing ZIP/APK/checksum assets. |
| Non-interactive release path | Non-TTY builds skip release publishing instead of offering dry-run/CI publish semantics. | `build.js:292` | CI cannot currently be the release authority without another wrapper. |
...
- 82 lexical build/website callables.
...
It pins 25 lexical callable rows across 9 semantic method groups: package copy,
