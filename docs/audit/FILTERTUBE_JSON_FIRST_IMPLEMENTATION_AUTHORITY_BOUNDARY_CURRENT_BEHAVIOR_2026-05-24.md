...
63 tracked JS/JSX/MJS files, 5,469 lexical callables, 0 files with complete
per-callable semantic proof, and 5,469 callables still requiring semantic proof
...
| `docs/audit/FILTERTUBE_METHOD_SEMANTIC_PROOF_GAP_INDEX_CURRENT_BEHAVIOR_2026-05-25.md` | 63 tracked JS/JSX/MJS files, 5,469 lexical callables, 0 files with complete per-callable semantic proof, and 5,469 callables still requiring semantic proof before JSON-first behavior changes. |
...
| `js/filter_logic.js:154` | runtime path syntax and unsupported documented syntax boundary |
| `js/filter_logic.js:426` | hand-authored renderer rule surface |
| `js/filter_logic.js:2126` | category metadata fetch trigger |
| `js/filter_logic.js:3434` | harvest before disabled skip and later mutation split |
...
```mermaid
flowchart TD
  Traffic["YouTube page traffic"]
  SeedGate["Seed/injector active-work gate"]
  PassThrough["Pass through before clone, parse, replay"]
  Engine["FilterTubeEngine.processData"]
  Rules["filter_logic renderer rules"]
  Harvest["Harvest and learned-map side effects"]
  Mutate["JSON hide/allow mutation"]
  Category["Category metadata fetch request"]
  DomParity["DOM fallback parity owner"]
  Actions["Menu and quick-block action lifecycle"]
  NoGo["First-class JSON implementation NO-GO"]

  Traffic --> SeedGate
  SeedGate --> PassThrough
  SeedGate --> Engine
  Engine --> Rules
  Rules --> Harvest
  Rules --> Mutate
...
| `json_flow_renderer_rule_owner` | `js/filter_logic.js:426-835` | `FILTER_RULES` owns current renderer key to field-path extraction for supported JSON-like renderer shapes. | Generated renderer owner manifest with endpoint, route, surface, supported/unsupported class, field effect, fixture, and doc provenance. |
| `json_flow_settings_list_mode_owner` | `js/filter_logic.js:938-1060` | Filter logic reconstructs serialized keyword regexes, list-mode settings, content filters, category filters, and channel rows before rule evaluation. | Product policy and fixtures for exact/substr, comments, empty whitelist, conflicts, unknown mode, and simultaneous allow/block states. |
| `json_flow_harvest_mutation_owner` | `js/filter_logic.js:3434-3494` | Engine processing harvests identity mappings before enabled-state mutation and separates `processData()` from `harvestOnly()`. | Harvest-versus-mutation budget with storage/map side-effect counters, disabled behavior, rollback, and no-rule proof. |
| `json_flow_category_metadata_owner` | `js/filter_logic.js:2126-2181`; `js/content_bridge.js:1794-1872` | Category rules can request video metadata when JSON row data lacks a category, delegating to a bridge fetch queue/cache. | Category fetch budget with cache hit/miss, credential class, retry/dedupe, network/storage counters, and false-hide proof. |
...
current method semantic gap lexical callables: 5469
