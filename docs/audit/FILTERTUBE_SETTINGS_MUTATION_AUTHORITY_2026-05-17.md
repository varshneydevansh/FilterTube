...
| Missing listType forwarding | `addFilteredChannel` message handler does not forward `message.listType` into `handleAddFilteredChannel(...)`. | The helper supports whitelist/blocklist targeting, but the runtime message path currently defaults to blocklist. |
...
| `js/background.js:5240` | background authority/actions | background auto backup | - | `scheduleAutoBackupInBackground((message.profile === 'kids') ? 'kids_channel_added' : 'channel_added');` | must only run after successful durable mutation |
