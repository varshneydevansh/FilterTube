...
| `js/content_bridge.js:6150` `initializeDOMFallback()` | Waits one second, applies DOM fallback only when managed viewing policy allows the route, installs fallback menu buttons, and sets up debounced mutation-driven fallback work. | `jsonFirstDomLifecycleBudget` before pruning or quieting DOM fallback due to JSON coverage. |
...
| `js/content_bridge.js:6554` `ensureFallbackMenuButtons()` | Installs fallback menu CSS and later owns menu repair listeners/observer/timer work outside the JSON engine. | `jsonFirstMenuLifecycleBudget` separating explicit user action affordances from passive filtering work. |
