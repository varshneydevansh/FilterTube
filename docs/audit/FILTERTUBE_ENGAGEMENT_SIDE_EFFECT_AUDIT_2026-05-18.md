...
| Current watch owner block | `enforceCurrentWatchOwnerBlock()` can pause the video, hide the selected playlist row, click a verified allowed playlist link, open a collapsed playlist panel, or retry DOM fallback. With no verified allowed successor, it keeps the current player blocked instead of clicking YouTube's generic Next button or hiding the watch shell. | A false owner match can still pause playback or change playlist navigation. | Exact owner-confidence fixture and selected-row policy before changing behavior. |
