...
- The parent-facing Quick Start, Exact keyword explanation, direct-access
  ...
  and the Help page.
- **Update refresh reminder preference**: Settings includes a device-wide
  opt-out control, **Disable refresh reminders after updates**, for the small
  ...
  only the first-run refresh prompt. The separate What’s New/release-notes
  banner remains available. See [Update Refresh Reminder Current Behavior](audit/FILTERTUBE_UPDATE_REFRESH_NOTIFICATION_RELEASE_SETTING_2026-08-23.md).
...
- **Family Device Updates**
  ...
  - use one Family Device map for nearby, same-network, and away/internet devices after pairing
  - use `Send Update` when both devices are open
  - use optional `Internet Pickup` only when a verified protected device should collect an unreadable pending update later or away from the parent device
  - use optional `Home Pickup` only when a home/school pickup service has been explicitly configured
- **Move full account**
