...
| Child profiles should not behave like normal admin surfaces | `Done` | Child profiles are receive-only from the child Accounts & Sync surface. Child PIN access can open receive-only sync, but sending, backups, parent controls, profile policy, and trusted-link policy stay parent/account-owned. | Preserve the reduced child surface as the default rule. |
...
| Need to prevent kids from modifying parent-decided rules | `Done` | Current lock gate blocks restricted views and mutations while locked; child Nanah role is receive-only; child surfaces block profile rename/delete/PIN mutation, account-policy changes, backup import/export, rule editing, and trusted parent-link edit/remove from Accounts & Sync. Parent/account profiles edit child Main/Kids rules from explicit child-row actions. | Preserve the defense-in-depth model; future additions should default closed for child surfaces. |
