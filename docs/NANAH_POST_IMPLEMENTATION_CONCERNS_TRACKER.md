...
| Need clarity on whether child-device behavior is child-profile-based or device-wide | `Partial` | Current rule is profile-based: if the active profile in Nanah is a child profile, Nanah becomes replica-only for that UI session. | Decide whether future device-wide child mode should exist beyond the active-profile rule. |
